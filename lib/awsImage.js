 ...
 */
async function recordUploadedImage(userId, uploadResult, options = {}) {
    const {
        seanceDate = null,
        seanceName = null,
        programId = null,
    ...
    const resolvedProgramId = programId && mongoose.Types.ObjectId.isValid(programId)
        ? programId
        : null;
...
async function recordProfilePicUpload(userId, uploadResult) {
    const savedImage = await recordUploadedImage(userId, uploadResult, { photoType: 'profile' });
 ...
 * @returns {Promise<Object>} - A promise that resolves to the recorded image object
 */
async function recordSeancePhotoUpload(userId, uploadResult, seanceDate, seanceName, programId = null, photoType = 'seance') {
    console.log("Uploading seance photo for user:", userId, "with seance date:", seanceDate, "and seance name:", seanceName, "programId:", programId, "photoType:", photoType, "with upload result:", uploadResult);
...
async function resolveSeanceNameFromProgramId(programId) {
    if (!programId || !mongoose.Types.ObjectId.isValid(programId)) return null;
    const program = await UserProgram.findById(programId).select('name').lean();
...
async function recordUpload(userId, uploadResult, options = {}) {
    ...
    const resolvedPhotoType = photoType || 'seance';

    if (resolvedPhotoType === 'avatarFace') {
        let resolvedProgramId = programId || null;
        if (resolvedProgramId) {
            await resolveSeanceNameFromProgramId(resolvedProgramId);
        }
        return recordSeancePhotoUpload(
            userId,
            uploadResult,
            seanceDate || new Date(),
            null,
    ...
    let resolvedSeanceName = seanceName;
    let resolvedProgramId = programId || null;
    if (resolvedProgramId) {
        const programName = await resolveSeanceNameFromProgramId(resolvedProgramId);
        if (programName) resolvedSeanceName = programName;
    }
 ...
 */
async function getSeanceImages(userId, seanceDate, seanceName, programId = null) {
    try {
        const query = { user: userId, seanceDate };
        if (programId && mongoose.Types.ObjectId.isValid(programId)) {
            query.programId = programId;
        } else if (seanceName) {
            query.seanceName = seanceName;
        } else {
            return [];
        }

        const images = await AwsImage.find(query).sort({ createdAt: -1 });
...
async function removeSeancePhoto(photoUrl) {
    try {
        ...
        if (image.photoType === 'avatarFace') {
            await Seance.updateMany(
                { avatarFaceUrl: photoUrl },
                { $set: { avatarFaceUrl: null } }
            );
        }

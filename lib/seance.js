...
const { updateLastSeanceId } = require('./programSeanceLink');
const { secondsToEquivalentReps } = require('../utils/oneRepMax');
const { buildRegularityDateQuery } = require('../utils/dateQuery');
...
function parseIdList(raw) {
    ...
    return list.map((id) => id.trim()).filter((id) => mongoose.Types.ObjectId.isValid(id));
}

function buildSortStage(sortBy, sortOrder) {
    const field = SORT_FIELDS[sortBy] || SORT_FIELDS.date;
    const dir = sortOrder === 'asc' ? 1 : -1;
    return { [field]: dir, _id: dir };
}

async function resolveProgramFilter(programIds, folderIds) {
    const hasProgramFilter = programIds.length > 0;
    const hasFolderFilter = folderIds.length > 0;
    if (!hasProgramFilter && !hasFolderFilter) return null;

    let allowedProgramIds = null;

    if (hasFolderFilter) {
        const folderObjectIds = folderIds.map((id) => new mongoose.Types.ObjectId(id));
        const folderPrograms = await UserProgram.find({ folder: { $in: folderObjectIds } })
            .select('_id')
            .lean();
        allowedProgramIds = new Set(folderPrograms.map((p) => p._id.toString()));
    }

    if (hasProgramFilter) {
        const programSet = new Set(programIds);
        if (allowedProgramIds) {
            allowedProgramIds = new Set([...allowedProgramIds].filter((id) => programSet.has(id)));
        } else {
            allowedProgramIds = programSet;
        }
    }
    ...
    return {
        $in: [...allowedProgramIds].map((id) => new mongoose.Types.ObjectId(id)),
    };
}
 ...
 */
async function getLastSeance(userId, field, seanceName, programId) {
    try {
        const query = { user: new mongoose.Types.ObjectId(userId) };
        if (programId && mongoose.Types.ObjectId.isValid(programId)) {
            query.program = new mongoose.Types.ObjectId(programId);
        } else if (seanceName) {
 ...
 * @param {Object} [options] - Optional filters and sort.
...
*/
async function getSeances(users, page = 1, limit = 3, seanceName, options = {}) {
    try {
        const skip = (page - 1) * limit;
        const {
            dateMin,
            dateMax,
            programIds: rawProgramIds,
        ...
        const dateFilter = buildRegularityDateQuery(dateMin, dateMax);
        if (Object.keys(dateFilter).length > 0) {
            query.date = dateFilter;
        }

        const programIds = parseIdList(rawProgramIds);
        const folderIds = parseIdList(rawFolderIds);
        const programFilter = await resolveProgramFilter(programIds, folderIds);
        if (programFilter) {
            query.program = programFilter;
        }

        const sortStage = buildSortStage(sortBy, sortOrder);
        const reactionsCollection = Reaction.collection.name;
        const commentsCollection = SeanceComment.collection.name;

        const [result] = await Seance.aggregate([
            { $match: query },
            {
                $lookup: {
                    from: reactionsCollection,
                    localField: '_id',
                    foreignField: 'seance',
                    as: '_reactionsRaw',
                },
                    ...
                    reactionCount: {
                        $size: {
                            $filter: {
                                ...
                                cond: {
                                    $or: [
                                        { $eq: ['$$reaction.comment', null] },
                                        { $eq: [{ $type: '$$reaction.comment' }, 'missing'] },
                                    ],
                                },
                            },
                        },
                    },
                    commentCount: { $size: '$_commentsRaw' },
                    prCount: {
                        $reduce: {
                            input: { $ifNull: ['$recordSummary', []] },
                            initialValue: 0,
                            in: { $add: ['$$value', { $ifNull: ['$$this.number', 0] }] },
                        },
                    },
                    totalSecondsSort: { $ifNull: ['$totalSeconds', 0] },
                    volumeSort: { $ifNull: ['$stats.totalWeight', 0] },
                },
            },
            {
                $facet: {
        ...
        const returnedCount = result.seances?.length || 0;
 ...
 */
async function resolveSeanceProgramPayload(seanceData, authenticatedUserId) {
    const payload = { ...seanceData, user: authenticatedUserId };
    const programId = payload.program || payload.programId;
    ...
    if (programId && mongoose.Types.ObjectId.isValid(programId)) {
        ...
        payload.program = userProgram._id;
        payload.name = userProgram.name;
    }
...
async function createSeance(seanceData, photoIds, authenticatedUserId) {
    try {
        ...
        Seance.init();
        const payload = await resolveSeanceProgramPayload(seanceData, authenticatedUserId);
        const newSeance = new Seance(payload);
        await newSeance.save();

        if (payload.program) {
            await updateLastSeanceId(payload.program, newSeance._id);
        }

        if (photoIds) {
            const photoUpdate = {
                seanceId: newSeance._id,
                seanceName: null,
                seanceDate: null,
            };
            if (payload.program) {
                photoUpdate.programId = payload.program;
            }
            ...
            const seancePhotos = seanceImages
                .filter((image) => image.photoType !== 'avatarFace')
                .map((image) => image.cloudfrontUrl);
            const avatarFaceImage = seanceImages.find((image) => image.photoType === 'avatarFace');

            await Seance.findByIdAndUpdate(
                newSeance._id,
                {
                    seancePhotos,
                    avatarFaceUrl: avatarFaceImage?.cloudfrontUrl || null,
                }
            );
        }
...
async function updateSeance(id, seanceData, photoIds, authenticatedUserId) {
    try {
        ...
        const resolved = await resolveSeanceProgramPayload(updateData, authenticatedUserId);
        Object.assign(updateData, resolved);
        ...
        if (photoIds) {
            ...
            if (updateData.program) {
                photoUpdate.programId = updateData.program;
            }
            await AwsImage.updateMany(
                { _id: { $in: photoIds } },
                { $set: photoUpdate }
            ...
            const seanceImages = await AwsImage.find({ _id: { $in: photoIds.map(id => new mongoose.Types.ObjectId(id)) } });
            updateData.seancePhotos = seanceImages
                .filter((image) => image.photoType !== 'avatarFace')
                .map((image) => image.cloudfrontUrl);
            const avatarFaceImage = seanceImages.find((image) => image.photoType === 'avatarFace');
            updateData.avatarFaceUrl = avatarFaceImage?.cloudfrontUrl || null;
        }
        ...
        if (updatedSeance.program) {
            await updateLastSeanceId(updatedSeance.program, updatedSeance._id);
        }

...
async function createSeance(seanceData) {
    try {
        ...
        await invalidateCacheStartingWith(`seance_${newSeance._id}`);
        await invalidateCacheStartingWith(`lastSeance_${seanceData.user}`);
        await invalidateCacheStartingWith(`user_stats_${seanceData.user}`);
...
async function deleteSeance(id, user) {
    try {
        ...
        // Invalidate relevant caches
        await invalidateCacheStartingWith(`seances_all`);
        await invalidateCacheStartingWith(`seances_${user}`);
        await invalidateCacheStartingWith(`seance_${id}`);
        await invalidateCacheStartingWith(`lastSeance_${user}`);
        await invalidateCacheStartingWith(`user_stats_${user}`);
    } catch (err) {
...
module.exports = { getLastSeance, getSeanceNames, getSeance, getSeances, createSeance, deleteSeance };

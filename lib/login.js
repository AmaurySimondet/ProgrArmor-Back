...
const googleAuthCodeSeen = new Map();
...
function trackSeenGoogleCode(codeHash) {
    if (!codeHash) return { seenCount: 0, firstSeenAt: null };
    const now = Date.now();
    for (const [key, meta] of googleAuthCodeSeen.entries()) {
        if ((now - meta.firstSeenTs) > 2 * 60 * 1000) {
            googleAuthCodeSeen.delete(key);
        }
    }
    const existing = googleAuthCodeSeen.get(codeHash);
    if (existing) {
        existing.count += 1;
        return { seenCount: existing.count, firstSeenAt: new Date(existing.firstSeenTs).toISOString() };
    }
    googleAuthCodeSeen.set(codeHash, { count: 1, firstSeenTs: now });
    return { seenCount: 1, firstSeenAt: new Date(now).toISOString() };
}
...
function google(req, res) {
    ...
    console.log("[google] Init", {
        requestId,
        redirectRaw: truncateForLog(req.query.redirect || ""),
        statePreview: truncateForLog(state, 80),
        host: req.headers.host,
        forwardedHost: req.headers["x-forwarded-host"],
        forwardedProto: req.headers["x-forwarded-proto"],
        userAgent: truncateForLog(req.headers["user-agent"] || "", 140)
    });
...
async function googleAuthenticate(req, res) {
    ...
    const codeHash = hashForLog(code);
    const codeTracking = trackSeenGoogleCode(codeHash);

    console.log("[googleAuthenticate] Called", {
        requestId,
        codePreview: maskValue(code),
        codeHash,
        codeSeenCount: codeTracking.seenCount,
        codeFirstSeenAt: codeTracking.firstSeenAt,
        statePreview: truncateForLog(state, 120),
        prompt: req.query.prompt || "",
        authuser: req.query.authuser || "",
        iss: req.query.iss || "",
        originalUrl: truncateForLog(req.originalUrl || "", 300),
        host: req.headers.host,
        forwardedHost: req.headers["x-forwarded-host"],
        forwardedProto: req.headers["x-forwarded-proto"]
    });
    try {
        const authenticate = () => new Promise((resolve, reject) => {
            console.log("[googleAuthenticate] Calling passport.authenticate('google')", { requestId });
            passport.authenticate("google", { session: false })(req, res, (err) => {
                if (err) {
                    console.error("[googleAuthenticate] Passport authenticate error:", {
                        ...
                        oauthErrorData: typeof err?.oauthError?.data === "string"
                            ? truncateForLog(err.oauthError.data, 500)
                            : err?.oauthError?.data
                    });
                    reject(err);
                }
                else {
                    console.log("[googleAuthenticate] Passport authenticate success", { requestId });
                    resolve();
                }
            });
        });
        ...
        await authenticate();
        console.log("[googleAuthenticate] Authenticated", {
            requestId,
            userId: req.user?._id
        });
        ...
        // Get redirect URL from state parameter
        const redirectUrl = req.query.state ? decodeURIComponent(req.query.state) : '/dashboard';
        console.log("[googleAuthenticate] Redirect URL", { requestId, redirectUrl: truncateForLog(redirectUrl, 180) });

        // Mobile deep links - redirect directly with token
        if (redirectUrl.match(/^(progarmor|exp):\/\//)) {
            const deepLink = `${redirectUrl}?token=${token}`;
            console.log("[googleAuthenticate] Detected mobile deep link", { requestId, deepLink: truncateForLog(deepLink, 220) });
            res.redirect(deepLink);
        } else {
            // Web - go through Token page
            const webRedirect = urlClient + `/token?token=${token}&redirect=${encodeURIComponent(redirectUrl)}`;
            console.log("[googleAuthenticate] Redirecting to web Token page", { requestId, webRedirect: truncateForLog(webRedirect, 220) });
        ...
        console.error('[googleAuthenticate] Google auth error:', {
            ...
            oauthErrorData: typeof error?.oauthError?.data === "string"
                ? truncateForLog(error.oauthError.data, 500)
...
async function forgotPassword(req, res) {
    try {
        ...
        // TODO: Plug your email service here (send `${urlClient}/reset-password?token=${rawToken}` by email).
        res.json({
            success: true,
            message: "If an account exists for this email, a reset link has been generated."
        });

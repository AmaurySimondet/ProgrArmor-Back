...
async function googleAuthenticate(req, res) {
    try {
        // Convert callback-style passport authenticate to Promise
        ...
        console.time('google-auth');
        await authenticate();
        console.timeEnd('google-auth');

        if (!req.user) {
            throw new Error('Authentication failed - no user');
        }

        // Perform lastLogin update and token generation in parallel
        console.time('db-and-token');

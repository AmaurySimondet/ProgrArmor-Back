...
async function login(req, res) {
    console.log(`[LOGIN] Attempt with body:`, { email: req.body?.email, ip: req.ip });
    const { email, password } = req.body;

    if (!email || !password) {
        console.log(`[LOGIN] Missing credentials. Provided: email=${!!email}, password=${!!password}`, { ip: req.ip });
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        if (!foundUser) {
            console.log(`[LOGIN] User not found for email=${email}`, { ip: req.ip });
            return res.status(401).json({
                success: false,
        ...
        passport.authenticate("local", async function (err, user) {
            if (err) {
                console.log(`[LOGIN] Passport auth error:`, { err, email, ip: req.ip });
            }
            if (err || !user) {
                console.log(`[LOGIN] Auth failed for email=${email} (user exist=${!!user})`, { ip: req.ip });
                return res.status(401).json({
                    success: false,
                    message: "Email ou mot de passe incorrect"
                });
            ...
            console.log(`[LOGIN] Success for user=${user._id}, email=${user.email}, ip=${req.ip}`);
            res.json({
                success: true,
        ...
        console.log(`[LOGIN] Exception:`, error, { email, ip: req.ip });

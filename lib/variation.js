    ...
    for (const combination of tokenCombinations) {
        const topVariationsForCombination = await Promise.all(combination.map(async (token, index) => {
            ...
            if (queryEmbedding) {
                ...
                const topVariations = variations.sort((a, b) => b.score - a.score)[0];
                return topVariations;
            }
            console.warn('Failed to generate embedding, falling back to regular search');
            const res = await getVariationBySearch(token, null, null, 1, 1);
        ...
        const finalScore = sumScore / combination.length + similarityScore;

        if (topVariationsForCombination.some(v => v && v.isExercice === true)) {
            allResults.push({
...
const getVariationByRRFSearch = async (search, type, options = {}) => {
    ...
    return await getOrSetCache(cacheKey, async () => {
        ...
        if (!queryEmbedding) {
            console.warn('Failed to generate embedding, falling back to regular search');
            const res = await getVariationBySearch(normalizedSearch, type, null, 1, 1).variations;
            console.log("res", res);
            return res;
        }

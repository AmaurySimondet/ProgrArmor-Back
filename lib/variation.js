 ...
 */
function recommendedVariationSortComparator(docA, docB, usageMap, isExercice, weights) {
...
async function getVariationBySearchRecommended(
    ...
    const [candidates, totalResult, usageRows, equivalentPeerMap] = await Promise.all([
        Variation.aggregate(searchStages).option({ maxTimeMS: SEARCH_MAX_TIME_MS }),
        Variation.aggregate([
            { $search: { index: 'variations', compound } },
            { $count: 'total' }
        ]).option({ maxTimeMS: SEARCH_MAX_TIME_MS }),
    ...
    return {
        variations: ranked.slice(skip, skip + safeLimit),
        total: totalResult.length > 0 ? totalResult[0].total : 0
    };
}

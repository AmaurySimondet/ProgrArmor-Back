...
async function enrichPerformedItemsWithCardVariations(tier1Items = []) {
    ...
    const [docs, compositionMap] = await Promise.all([
        ...
        loadVerifiedExerciseCompositionSignatureMap()
    ]);
    ...
    return tier1Items.map((item) => {
        ...
        const variationDocs = orderedIds
            .map((id) => byId.get(id))
            .filter(Boolean);
        let cardVariation = null;
        if (variationDocs.length > 1) {
            cardVariation = { variations: variationDocs };
        } else if (variationDocs.length === 1) {
            cardVariation = variationDocs[0];
        } else {
            const repId = item?.performedRow?.variationId;
            cardVariation = repId ? byId.get(String(repId)) || null : null;
        }
        const localPicture = getPreferredPictureFromCardVariation(cardVariation);
        const resolvedPicture = localPicture
            || resolvePictureFromLoggedVariationIds(orderedIds, signatureMap, maxEquivLen)
            || null;

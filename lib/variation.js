...
const { buildVariationSearchCompound, mergeQueryWithSelfmadeVisibility, isVariationVisibleToUser } = require('./variationSearchPipelines');
...
const { NORMAL_FLOW_FAMILY_MAX_DEPTH } = require('../constants').set;
const RECOMMENDED_CONTEXT_TEXT_WEIGHT = 0.8;
const RECOMMENDED_CONTEXT_EQUIVALENT_WEIGHT = 0.2;
const WORKOUT_DETAIL_TIER1_MAX = 30;
/** Nombre max de combos affichés avant les exercices equivalentTo / nom (page 1 visible). */
const WORKOUT_DETAIL_TIER1_FIRST_BATCH = 5;
const WORKOUT_DETAIL_TIER1_WEIGHTS = {
    equivalentTo: 0.5,
    name: 0.25,
    count: 0.25
};
...
function getSearchableTokens(doc) {
    ...
    aliases.forEach((alias) => {
        tokenizeExact(alias).forEach((token) => tokens.add(token));
    });
    return tokens;
}
...
function hasExactTokenMatch(variation, searchTokens = []) {
    ...
    const itemTokens = getSearchableTokens(variation);
...
function variationMatchesWeightType(entry, weightType) {
    if (!weightType) return true;
    const hasWeightType = (variationDoc) => variationDoc?.weightType === weightType;

    if (Object.prototype.hasOwnProperty.call(entry || {}, 'weightType')) {
        return hasWeightType(entry);
    }
    ...
    return entry.variations.some(hasWeightType);
}
...
const getVariationBySearchBase = async (search, type, sortBy, page, limit, verified, isExercice, detailWeightType, muscle, weightType, userId) => {
    ...
    const compound = buildVariationSearchCompound({ search, type, verified, isExercice, muscle, weightType });
    const selfmadeMatch = mergeQueryWithSelfmadeVisibility({}, userId);
    const postSearchStages = [
        { $match: selfmadeMatch },
...
const getVariationBySearch = async (
    ...
    if (normalizedSortBy === 'recommended' && userId) {
        searchResult = await getVariationBySearchRecommended(
            search,
            type,
            page,
            limit,
    ...
    } else if (normalizedSortBy === 'recommended' && !userId) {
        searchResult = await getVariationBySearchBase(
            search,
            type,
            'popularity',
            page,
        ...
        if (!shouldMergeMyExercises) {
            ...
            const [regularSearch, myExercisesSearch] = await Promise.all([
                getVariationBySearchBase(search, type, normalizedSortBy, 1, candidateLimit, verified, isExercice, detailWeightType, muscle, weightType, userId),
            ...
            const filteredMyExercises = myExercisesSearch.variations.filter((item) => {
                ...
                if (weightType && !variationMatchesWeightType(item, weightType)) return false;
                return true;
            });

            searchResult = weightedRrfMerge(regularSearch.variations, filteredMyExercises, {
                page,
                limit,
    ...
    if (!userId) return searchResult;

    const usageRows = await aggregateAllSlotUsageByVariation(new mongoose.Types.ObjectId(userId));
    const usageMap = new Map(usageRows.map((row) => [row._id.toString(), row.count]));
    return {
        ...searchResult,
        variations: injectUserUsageCountInVariations(searchResult.variations, usageMap)
    };
};
...
const getAllVariationsBase = async (type, sortBy, userId, page = 1, limit = 20, verified, isExercice, detailWeightType) => {
    const baseQuery = {};
    if (type) {
        baseQuery.type = new mongoose.Types.ObjectId(type);
    }
    if (verified !== undefined) {
        baseQuery.verified = verified;
    }
    if (isExercice !== undefined) {
        baseQuery.isExercice = isExercice;
    }
    const query = mergeQueryWithSelfmadeVisibility(baseQuery, userId);
 ...
 */
function recommendedVariationSortComparator(
    docA,
    docB,
    usageMap,
    isExercice,
    weights,
    searchSignalMaps = null,
    exactUsageMap = null
) {
    ...
    const ea = idA && exactUsageMap instanceof Map ? Number(exactUsageMap.get(idA) || 0) : 0;
    const eb = idB && exactUsageMap instanceof Map ? Number(exactUsageMap.get(idB) || 0) : 0;
    const usedA = na > 0 || ea > 0;
    const usedB = nb > 0 || eb > 0;
    if (usedA !== usedB) {
    ...
    if (usedA && usedB) {
        ...
        const exactScoreA = Math.log1p(ea);
        const exactScoreB = Math.log1p(eb);
        if (exactScoreB !== exactScoreA) return exactScoreB > exactScoreA ? 1 : -1;
...
function rankVariationsForRecommended(
    ...
    const ranked = [...variations].sort((docA, docB) =>
        recommendedVariationSortComparator(
            docA,
            docB,
            usageByVariation,
            isExercice,
...
async function loadContextEquivalentExerciseRankedList({ contextVariationId, type, verified, muscle, weightType, limit, userId }) {
    ...
    const contextVariationById = await set.loadVariationByIdClosure(
        [rootDoc._id.toString(), ...familySeedIds],
        { equivalentTo: 1 },
    );
    const contextExpandedLeaves = set.resolveExpandedLeafVariationIds(
        [rootDoc._id.toString()],
        contextVariationById,
    );
    ...
    const getPrefixDepthFromEquivalentTo = (equivalentTo = []) => {
        ...
        if (contextExpandedLeaves.length >= 2
            && contextExpandedLeaves.every((leafId) => equivalentSet.has(leafId))) {
            return contextExpandedLeaves.length;
        }
        let bestDepth = 0;
    ...
    if (contextExpandedLeaves.length >= 2) {
        orFilters.push({
            equivalentTo: {
                $all: contextExpandedLeaves.map((id) => new mongoose.Types.ObjectId(id)),
            },
        });
    }
    ...
    if (weightType && typeof weightType === 'string') {
        match.weightType = weightType;
    }

    const finalMatch = mergeQueryWithSelfmadeVisibility(match, userId);
...
async function loadVerifiedExerciseCompositionSignatureMap() {
    const docs = await Variation.find(
        ...
        { equivalentTo: 1, type: 1, popularity: 1, createdAt: 1, picture: 1 }
    )
        .sort({ popularity: -1, createdAt: 1 })
        .lean();
    ...
    for (const d of docs) {
        ...
        if (!signatureMap.has(sig)) {
            signatureMap.set(sig, {
                ...
                equivalentToIds: list.map((id) => String(id))
            });
        }
    }
...
const COMPOSITION_SIGNATURE_MAP_TTL_MS = 5 * 60 * 1000;
let compositionSignatureMapCache = null;
let compositionSignatureMapExpiresAt = 0;

async function getVerifiedExerciseCompositionSignatureMap() {
    const now = Date.now();
    if (compositionSignatureMapCache && now < compositionSignatureMapExpiresAt) {
        return compositionSignatureMapCache;
    }
    compositionSignatureMapCache = await loadVerifiedExerciseCompositionSignatureMap();
    compositionSignatureMapExpiresAt = now + COMPOSITION_SIGNATURE_MAP_TTL_MS;
    return compositionSignatureMapCache;
}

function buildCardVariationFromOrderedIds(orderedIds, byId, fallbackVariationId = null) {
    const variationDocs = (orderedIds || [])
        .map((id) => byId.get(String(id)))
        .filter(Boolean);
    if (variationDocs.length > 1) return { variations: variationDocs };
    if (variationDocs.length === 1) return variationDocs[0];
    if (fallbackVariationId) return byId.get(String(fallbackVariationId)) || null;
    return null;
}

function resolvePictureFromCardAndComposition(orderedIds, cardVariation, signatureMap, maxEquivLen) {
    const localPicture = getPreferredPictureFromCardVariation(cardVariation);
    if (localPicture) return { picture: localPicture, source: 'embedded' };
    const compositionPicture = resolvePictureFromLoggedVariationIds(orderedIds, signatureMap, maxEquivLen);
    if (compositionPicture) return { picture: compositionPicture, source: 'composition' };
    return { picture: null, source: null };
}

/**
 * Résout l’image d’une combinaison de variations (ordre préservé).
 * @param {string[]} orderedIds
 * @param {{ byId?: Map, compositionMap?: { signatureMap, maxEquivLen }, fallbackVariationId?: string }} [options]
 */
async function resolvePictureForVariationIds(orderedIds, options = {}) {
    const ids = (orderedIds || []).map((id) => String(id)).filter(Boolean);
    if (!ids.length) return { picture: null, source: null };

    const validOids = ids
        .filter((id) => mongoose.Types.ObjectId.isValid(id))
        .map((id) => new mongoose.Types.ObjectId(id));
    if (!validOids.length) return { picture: null, source: null };

    const [docs, compositionMap] = await Promise.all([
        options.byId
            ? Promise.resolve([...options.byId.values()])
            : Variation.aggregate([
                { $match: { _id: { $in: validOids } } },
                ...lookup_type,
            ]).option({ maxTimeMS: SEARCH_MAX_TIME_MS }),
        options.compositionMap ?? getVerifiedExerciseCompositionSignatureMap(),
    ]);

    const byId = options.byId ?? new Map(docs.map((doc) => [String(doc._id), doc]));
    const { signatureMap, maxEquivLen } = compositionMap;
    const cardVariation = buildCardVariationFromOrderedIds(
        ids,
        byId,
        options.fallbackVariationId ?? null,
    );

    return resolvePictureFromCardAndComposition(ids, cardVariation, signatureMap, maxEquivLen);
}

/**
 * Photo d’un combo loggé : plus long equivalentTo vérifié contenu dans le set (puis préfixe ordonné).
 */
function resolvePictureFromLoggedVariationIds(orderedIds, signatureMap, maxEquivLen) {
    const ids = (orderedIds || []).map((id) => String(id)).filter(Boolean);
    if (!ids.length || !signatureMap?.size) return null;

    const performedSet = new global.Set(ids);
    let bestPicture = null;
    let bestLen = 0;

    for (const [, canon] of signatureMap.entries()) {
        if (!canon?.picture) continue;
        const parts = getSortedVariationIds(
            canon.equivalentToIds || []
        );
        if (!parts.length || parts.length > maxEquivLen) continue;
        if (parts.every((id) => performedSet.has(id)) && parts.length > bestLen) {
            bestLen = parts.length;
            bestPicture = canon.picture;
        }
    }
    if (bestPicture) return bestPicture;
    ...
    for (let k = kMax; k >= 1; k--) {
        ...
        if (canon?.picture) return canon.picture;
    }
    return null;
}

function getPreferredPictureFromVariationDocs(variationDocs = []) {
    const exercisePic = variationDocs.find((doc) => doc?.isExercice === true && doc?.picture)?.picture;
    if (exercisePic) return exercisePic;
    return variationDocs.find((doc) => doc?.picture)?.picture || null;
}

function getPreferredPictureFromCardVariation(cardVariation) {
    if (!cardVariation) return null;
    if (Array.isArray(cardVariation?.variations)) {
        return getPreferredPictureFromVariationDocs(cardVariation.variations);
    }
    return cardVariation?.picture || null;
}
 ...
 */
function resolveExercisePrimaryFromLoggedVariations(variations, signatureMap, maxEquivLen, expansionById = null) {
    if (!variations?.length) return null;
    const ids = variations.map((v) => v?.variation?.toString()).filter(Boolean);
    if (!ids.length) return null;

    if (expansionById) {
        const expandedIds = set.resolveExpandedLeafVariationIds(ids, expansionById);
        const expandedSig = getVariationSignature(expandedIds);
        const canonFromExpanded = signatureMap.get(expandedSig);
        if (canonFromExpanded) {
            return {
                variationId: canonFromExpanded._id.toString(),
                typeId: canonFromExpanded.type ? canonFromExpanded.type.toString() : null,
            };
        }
    }
...
async function loadVariationExpansionByIdMap() {
    const docs = await Variation.find(
        { 'equivalentTo.1': { $exists: true } },
        { equivalentTo: 1 },
    ).lean();
    const seedIds = docs.flatMap((doc) => [
        String(doc._id),
        ...(Array.isArray(doc.equivalentTo) ? doc.equivalentTo.map((id) => String(id)) : []),
    ]);
    return set.loadVariationByIdClosure(seedIds, { equivalentTo: 1 });
}
...
async function aggregateExercisePrimaryUsageFacet(userObjectId) {
    const { signatureMap, maxEquivLen } = await getVerifiedExerciseCompositionSignatureMap();
    const expansionById = await loadVariationExpansionByIdMap();
...
async function aggregateAllSlotUsageByVariation(userObjectId) {
...
function injectUserUsageCountInVariations(variations, usageMap, recommendedUsageMap = null) {
    return (variations || []).map((variationDoc) => {
        const id = variationDoc?._id?.toString();
        const userUsageCountExact = id ? Number(usageMap?.get(id) || 0) : 0;
        const hasRecommendedMap = recommendedUsageMap instanceof Map;
        const userUsageCountRecommended = id
            ? hasRecommendedMap
                ? Number(recommendedUsageMap.get(id) || 0)
                : Number(variationDoc?.userUsageCountRecommended ?? userUsageCountExact)
            : 0;
        return {
            ...variationDoc,
            userUsageCountExact,
            userUsageCountRecommended
        };
    });
}

function injectUserUsageCountInGroups(groups, usageMap, recommendedUsageMap = null) {
    return (groups || []).map((group) => ({
        ...group,
        variations: injectUserUsageCountInVariations(
            group?.variations || [],
            usageMap,
            recommendedUsageMap
        )
...
async function getVariationBySearchRecommended(
    ...
    const exactUsageRows = await aggregateAllSlotUsageByVariation(userObjectId);
    ...
    for (const doc of candidates) {
        ...
        const itemTokens = getSearchableTokens(doc);
    ...
    const exactUsageMap = new Map(exactUsageRows.map((r) => [r._id.toString(), r.count]));
    ...
    const paginatedVariations = rankedWithContextFusion.slice(skip, skip + safeLimit);

    return {
        variations: injectUserUsageCountInVariations(paginatedVariations, exactUsageMap, usageForScore),
...
const getVariationsGroupedByType = async (
    ...
    let userUsageCountMap = null;
    const ensureUserUsageCountMap = async () => {
        if (!userId) return null;
        if (userUsageCountMap) return userUsageCountMap;
        const usageRows = await aggregateAllSlotUsageByVariation(new mongoose.Types.ObjectId(userId));
        userUsageCountMap = new Map(usageRows.map((row) => [row._id.toString(), row.count]));
        return userUsageCountMap;
    };
    ...
    if (normalizedSortBy === 'recommended' && userId) {
        ...
        const [usageFacet, groupsRaw, equivalentPeerMap, exactUsageRows] = await Promise.all([
            isExercice === false
                ? SeanceSet.aggregate(
            ...
            aggregateAllSlotUsageByVariation(userObjectId)
        ]);
        ...
        const exactUsageByVariation = new Map(
            (exactUsageRows || []).map((row) => [row._id.toString(), row.count])
        );
        if (isExercice === true) {
        ...
        const groups = hasTypesPagination
            ...
            : groupsAll;
        const usageMap = exactUsageByVariation;

        return {
            groups: injectUserUsageCountInGroups(groups, usageMap, usageByVariation),
    ...
    if ((normalizedSortBy === 'frequency' || normalizedSortBy === 'lastUsed') && userId) {
        ...
        const groups = hasTypesPagination
            ? groupsAll.slice(typeSkip, typeSkip + normalizedTypesLimit)
            : groupsAll;
        const usageMap = await ensureUserUsageCountMap();

        return {
            groups: injectUserUsageCountInGroups(groups, usageMap),
            totalTypes: groupsAll.length
        };
    }
...
const getVariationEquivalents = async (variationId, maxLevel = 3) => {
    ...
    if (maxLevel >= 0) {
        ...
        const inputById = await set.loadVariationByIdClosure([inputId.toString()], { equivalentTo: 1 });
        const inputExpandedSignature = getVariationSignature(
            set.resolveExpandedLeafVariationIds([inputId.toString()], inputById),
        );
        if (inputExpandedSignature) {
            const expandedLeafIds = inputExpandedSignature.split('|').filter(Boolean);
            const expandedObjectIds = expandedLeafIds.map((id) => new mongoose.Types.ObjectId(id));
            const expandedCandidates = await Variation.find(
                {
                    _id: { $ne: inputId },
                    'equivalentTo.1': { $exists: true },
            ...
            for (const doc of expandedCandidates) {
                const docId = doc._id.toString();
                if (assigned.has(docId)) continue;
                const docExpandedSignature = getVariationSignature(
                    set.resolveExpandedLeafVariationIds([docId], inputById),
                );
                if (docExpandedSignature === inputExpandedSignature) {
                    directIds.add(docId);
                    assigned.add(docId);
                }
            }
...
function buildPerformedWorkoutSuggestionItems(families = [], performedVariationsByFamily = {}) {
    const depthByKey = new Map(
        (families || []).map((family) => [String(family?.familyKey || ''), Number(family?.depth || 0)])
    );
    const items = [];
    for (const [familyKey, rows] of Object.entries(performedVariationsByFamily || {})) {
        if (!Array.isArray(rows)) continue;
        for (const row of rows) {
            const variationIds = Array.isArray(row?.variations) && row.variations.length > 0
                ? row.variations.map((id) => String(id)).filter(Boolean)
                : (
                    Array.isArray(row?.scopeVariationIds) && row.scopeVariationIds.length > 0
                        ? row.scopeVariationIds.map((id) => String(id)).filter(Boolean)
                        : [String(row?.variationId || '')].filter(Boolean)
                );
            if (!variationIds.length) continue;
            const progressionSignature = row?.progressionSignature
                || set.getVariationSignature(variationIds);
            const chartSourceVariationSignature = row?.chartSourceVariationSignature || progressionSignature;
            const labelValue = row?.name;
            const label = typeof labelValue === 'string'
                ? labelValue
                : (labelValue?.fr || labelValue?.en || variationIds.join(' + '));
            items.push({
                kind: 'performed',
                tier: 1,
                familyKey: String(familyKey),
                familyDepth: depthByKey.get(String(familyKey)) || 0,
                label,
                performedRow: row,
                variationIds,
    ...
    for (const item of items) {
        const signatureKey = String(item.progressionSignature || '');
        if (!signatureKey) continue;
        const existing = dedupedBySignature.get(signatureKey);
        if (!existing || item.count > existing.count || (item.count === existing.count && item.familyDepth > existing.familyDepth)) {
            dedupedBySignature.set(signatureKey, item);
        }
    }
    const uniqueItems = [...dedupedBySignature.values()];
    uniqueItems.sort((a, b) => {
        if (b.count !== a.count) return b.count - a.count;
        if (b.familyDepth !== a.familyDepth) return b.familyDepth - a.familyDepth;
        return String(a.label || '').localeCompare(String(b.label || ''), 'fr');
    });
    return uniqueItems;
}

function collectPerformedVariationIdSet(item) {
    const ids = new global.Set((item?.variationIds || []).map((id) => String(id)));
    const row = item?.performedRow;
    if (row?.variationId) ids.add(String(row.variationId));
    if (Array.isArray(row?.equivalentTo)) {
        for (const id of row.equivalentTo) ids.add(String(id));
    }
    if (Array.isArray(row?.scopeVariationIds)) {
        for (const id of row.scopeVariationIds) ids.add(String(id));
    }
    return ids;
}

function getContextPrefixOverlapScore(variationIds, familySeedIds, contextVariationId = null) {
    const performedSet = new global.Set((variationIds || []).map((id) => String(id)));
    const seeds = (familySeedIds || []).map((id) => String(id)).filter(Boolean);
    if (!seeds.length) return 0;

    let consecutiveDepth = 0;
    for (const id of seeds) {
        if (performedSet.has(id)) consecutiveDepth += 1;
        else break;
    }
    const consecutiveScore = consecutiveDepth / seeds.length;
    const intersection = seeds.filter((id) => performedSet.has(id)).length;
    const densityScore = intersection / seeds.length;

    const prefixes = set.buildVariationPrefixes(seeds, seeds.length);
    let bestPrefixFit = 0;
    for (const prefix of prefixes) {
        if (prefix.every((id) => performedSet.has(String(id)))) {
            bestPrefixFit = Math.max(bestPrefixFit, prefix.length / seeds.length);
        }
    }

    let score = Math.max(consecutiveScore, densityScore, bestPrefixFit);
    if (contextVariationId && performedSet.has(String(contextVariationId))) {
        score = Math.min(1, score + 0.35);
    }
    return Math.min(1, score);
}

function getPerformedSuggestionCountScore(count, maxCount) {
    const c = Math.max(0, Number(count || 0));
    const max = Math.max(1, Number(maxCount || 0));
    if (c <= 0) return 0;
    return Math.log1p(c) / Math.log1p(max);
}

function getPerformedSuggestionNameTokens(item) {
    const tokens = new Set();
    tokenizeExact(String(item?.label || '')).forEach((token) => tokens.add(token));
    return tokens;
}

function getPerformedSuggestionNameScore(item, searchTokens = []) {
    if (!searchTokens.length) return 0;
    const itemTokens = getPerformedSuggestionNameTokens(item);
    const matchCount = searchTokens.reduce(
        (acc, token) => acc + (itemTokens.has(token) ? 1 : 0),
        0
    );
    const coverage = matchCount / searchTokens.length;
    if (searchTokens.length >= 2 && coverage < 1) {
        return coverage * 0.65;
    }
    return coverage;
}

function scorePerformedWorkoutSuggestionItem(item, context = {}) {
    const weights = context.weights || WORKOUT_DETAIL_TIER1_WEIGHTS;
    const maxCount = Math.max(1, Number(context.maxCount || 0));
    const performedIds = [...collectPerformedVariationIdSet(item)];
    const eqScore = getContextPrefixOverlapScore(
        performedIds,
        context.familySeedIds,
        context.contextVariationId
    );
    const nameScore = getPerformedSuggestionNameScore(item, context.searchTokens);
    const countScore = getPerformedSuggestionCountScore(item?.count, maxCount);
    return (
        weights.equivalentTo * eqScore +
        weights.name * nameScore +
        weights.count * countScore
    );
}

function rankPerformedWorkoutSuggestionItems(items = [], { search, familySeedIds, contextVariationId } = {}) {
    if (!Array.isArray(items) || items.length === 0) return [];
    const searchTokens = tokenizeExact(String(search || '').trim());
    const maxCount = Math.max(...items.map((item) => Number(item?.count || 0)), 1);
    const scored = items.map((item) => ({
        item,
        score: scorePerformedWorkoutSuggestionItem(item, {
            searchTokens,
            familySeedIds,
            maxCount,
            contextVariationId
    ...
    scored.sort((a, b) => {
        ...
        if (b.item.familyDepth !== a.item.familyDepth) return b.item.familyDepth - a.item.familyDepth;
        return String(a.item.label || '').localeCompare(String(b.item.label || ''), 'fr');
    });
    return scored.map((entry) => entry.item);
}

async function enrichPerformedItemsWithCardVariations(tier1Items = []) {
    if (!Array.isArray(tier1Items) || tier1Items.length === 0) return [];

    const allIds = new Set();
    for (const item of tier1Items) {
        for (const id of item?.variationIds || []) allIds.add(String(id));
        if (item?.performedRow?.variationId) allIds.add(String(item.performedRow.variationId));
    }
    const validOids = [...allIds]
        .filter((id) => mongoose.Types.ObjectId.isValid(id))
        .map((id) => new mongoose.Types.ObjectId(id));
    if (!validOids.length) return tier1Items;
    ...
    const [docs, compositionMap] = await Promise.all([
        ...
        getVerifiedExerciseCompositionSignatureMap()
    ]);
    const byId = new Map(docs.map((doc) => [String(doc._id), doc]));
    const { signatureMap, maxEquivLen } = compositionMap;

    return tier1Items.map((item) => {
        const orderedIds = (item?.variationIds || []).map((id) => String(id)).filter(Boolean);
        const cardVariation = buildCardVariationFromOrderedIds(
            orderedIds,
            byId,
            item?.performedRow?.variationId ?? null,
        );
        const { picture: resolvedPicture } = resolvePictureFromCardAndComposition(
            orderedIds,
            cardVariation,
            signatureMap,
            maxEquivLen,
        );
        return { ...item, cardVariation, resolvedPicture };
    });
}

function buildWorkoutSuggestionsOrderedList(tier1Items, tier2Items, tier3Items) {
    const leadingTier1 = tier1Items.slice(0, WORKOUT_DETAIL_TIER1_FIRST_BATCH);
    const trailingTier1 = tier1Items.slice(WORKOUT_DETAIL_TIER1_FIRST_BATCH);
    if (trailingTier1.length === 0) {
        return [...leadingTier1, ...tier2Items, ...tier3Items];
    }
    return [...tier1Items, ...tier2Items, ...tier3Items];
}

function collectCatalogComboSignatures(exerciseSuggestionItems = []) {
    const signatures = new Set();
    for (const entry of exerciseSuggestionItems || []) {
        const doc = entry?.variation;
        if (!doc) continue;
        const equivalentTo = Array.isArray(doc?.equivalentTo)
            ? doc.equivalentTo.map((id) => String(id)).filter(Boolean)
            : [];
        if (equivalentTo.length >= 2) {
            signatures.add(set.getVariationSignature(equivalentTo));
        }
    }
    return signatures;
}

function dropPerformedCoveredByCatalogCombos(tier1Items = [], catalogComboSignatures = new Set()) {
    if (!catalogComboSignatures?.size) return tier1Items;
    return tier1Items.filter((item) => {
        const progressionSignature = String(item?.progressionSignature || '');
        const chartSignature = String(item?.chartSourceVariationSignature || '');
        return !catalogComboSignatures.has(progressionSignature) && !catalogComboSignatures.has(chartSignature);
    });
}

function buildWorkoutDetailExclusionState(performedItems = [], { includeComboSignatures = true } = {}) {
    const variationIds = new Set();
    const signatures = new Set();
    for (const item of performedItems) {
        if (includeComboSignatures) {
            if (item?.progressionSignature) signatures.add(String(item.progressionSignature));
            if (item?.chartSourceVariationSignature) signatures.add(String(item.chartSourceVariationSignature));
        }
        for (const id of item?.variationIds || []) variationIds.add(String(id));
        if (item?.performedRow?.variationId) variationIds.add(String(item.performedRow.variationId));
    }
    return { variationIds, signatures };
}

async function buildWorkoutSelectionFilterFromIds(selectedVariationIds = []) {
    const selectedIds = new Set(
        (Array.isArray(selectedVariationIds) ? selectedVariationIds : [])
            .map((id) => String(id))
            .filter(Boolean)
    );
    const signatures = new Set();
    if (selectedIds.size > 0) {
        signatures.add(set.getVariationSignature([...selectedIds]));
    }
    const validOids = [...selectedIds]
        .filter((id) => mongoose.Types.ObjectId.isValid(id))
        .map((id) => new mongoose.Types.ObjectId(id));
    if (validOids.length > 0) {
        const variationById = await set.loadVariationByIdClosure([...selectedIds], { equivalentTo: 1 });
        const expandedIds = set.resolveExpandedLeafVariationIds([...selectedIds], variationById);
        if (expandedIds.length > 0) {
            signatures.add(set.getVariationSignature(expandedIds));
        }
        for (const doc of variationById.values()) {
            const equivalentTo = Array.isArray(doc?.equivalentTo)
                ? doc.equivalentTo.map((id) => String(id)).filter(Boolean)
                : [];
            if (equivalentTo.length >= 2) {
    ...
    return { selectedIds, signatures };
}

function isWorkoutSuggestionMatchingSelection(item, selectionFilter) {
    if (!item || !selectionFilter) return false;
    const { selectedIds, signatures } = selectionFilter;
    if (item?.kind === 'performed') {
        const progressionSignature = String(item?.progressionSignature || '');
        const chartSignature = String(item?.chartSourceVariationSignature || '');
        if (signatures.has(progressionSignature) || signatures.has(chartSignature)) return true;
        return false;
    }
    if (item?.kind === 'exercise') {
        const variationId = item?.variation?._id?.toString();
        if (variationId && selectedIds.has(variationId)) return true;
        const equivalentTo = Array.isArray(item?.variation?.equivalentTo)
            ? item.variation.equivalentTo.map((id) => String(id)).filter(Boolean)
            : [];
        if (equivalentTo.length >= 2) {
            const equivalentSignature = set.getVariationSignature(equivalentTo);
            if (signatures.has(equivalentSignature)) return true;
        }
    }
    return false;
}

function filterWorkoutSuggestionsBySelection(suggestions = [], selectionFilter) {
    if (!selectionFilter?.selectedIds?.size && !selectionFilter?.signatures?.size) {
        return suggestions;
    }
    return suggestions.filter((item) => !isWorkoutSuggestionMatchingSelection(item, selectionFilter));
}

function isWorkoutExerciseSuggestionExcluded(doc, exclusionState, extraVariationIds = null) {
    const id = doc?._id?.toString();
    if (!id) return true;
    if (exclusionState?.variationIds?.has(id)) return true;
    if (extraVariationIds?.has(id)) return true;
    const equivalentTo = Array.isArray(doc?.equivalentTo)
        ? doc.equivalentTo.map((entry) => String(entry)).filter(Boolean)
        : [];
    if (equivalentTo.length >= 2) {
        const sig = set.getVariationSignature(equivalentTo);
        if (exclusionState?.signatures?.has(sig)) return true;
    }
    return false;
}

async function getWorkoutDetailNameSearchExercises({
    search,
    userId,
    weightType,
    type,
    ...
    const variationWeights = resolveRecommendedVariationWeights();
    const safeCandidateLimit = Math.max(1, Number(candidateLimit || GROUPED_BY_TYPE_RECOMMENDED.VARIATION_SEARCH_CANDIDATE_LIMIT));
    const compound = buildVariationSearchCompound({
    ...
    const [candidates, usageRows, equivalentPeerMap] = await Promise.all([
        Variation.aggregate(searchStages).option({ maxTimeMS: SEARCH_MAX_TIME_MS }),
        aggregatePrimarySlotUsageByVariation(userObjectId),
        loadExerciseEquivalentPeerMap()
    ]);
    const exactUsageRows = await aggregateAllSlotUsageByVariation(userObjectId);

    const searchTokens = tokenizeExact(normalizedSearchInput);
    const maxAtlasScore = candidates.reduce((max, doc) => {
        const v = Number(doc?.atlasSearchScore || 0);
        return v > max ? v : max;
    }, 0);
    ...
    const filteredCandidates = searchTokens.length >= 2
        ...
        : candidates;
    const rankingBase = (filteredCandidates.length > 0 ? filteredCandidates : candidates)
        .filter((doc) => !isWorkoutExerciseSuggestionExcluded(doc, exclusionState, extraVariationIds));

    const usageMap = new Map(usageRows.map((r) => [r._id.toString(), r.count]));
    const exactUsageMap = new Map(exactUsageRows.map((r) => [r._id.toString(), r.count]));
    const usageForScore = expandRecommendedVariationUsageWithSpillover(
        usageMap,
        equivalentPeerMap,
        GROUPED_BY_TYPE_RECOMMENDED.VARIATION_EQUIVALENT_SPILLOVER_WEIGHT
    );
...
/**
 * Suggestions workout (flux détail) : tier 1 familles performées, tier 2 equivalentTo, tier 3 nom.
 */
async function getWorkoutDetailSuggestions({
    userId,
    contextVariationId,
    search,
    weightType,
    ...
    if (!userId || !contextVariationId) {
        return emptyPayload;
    }
    if (!mongoose.Types.ObjectId.isValid(String(contextVariationId))) {
        return emptyPayload;
    }
    ...
    const maxDepthApplied = Number.isFinite(depthInput)
        ? Math.max(1, Math.min(Math.floor(depthInput), NORMAL_FLOW_FAMILY_MAX_DEPTH))
        : NORMAL_FLOW_FAMILY_MAX_DEPTH;

    const familyPayload = await set.getNormalFlowPerformedVariationFamilies({
        userId,
        variations: [String(contextVariationId)],
        maxDepth: maxDepthApplied,
        dateMin: null
    });

    const families = Array.isArray(familyPayload?.families) ? familyPayload.families : [];
    const performedVariationsByFamily = familyPayload?.performedVariationsByFamily && typeof familyPayload.performedVariationsByFamily === 'object'
        ? familyPayload.performedVariationsByFamily
        : {};

    const allTier1Items = buildPerformedWorkoutSuggestionItems(families, performedVariationsByFamily);
    const familySeedIds = Array.isArray(familyPayload?.meta?.familySeedVariations)
        ? familyPayload.meta.familySeedVariations.map((id) => String(id)).filter(Boolean)
        : [];
    const rankedTier1Items = rankPerformedWorkoutSuggestionItems(allTier1Items, {
        search,
        familySeedIds,
        contextVariationId: String(contextVariationId)
    });
    const exclusionStateIdsOnly = buildWorkoutDetailExclusionState(rankedTier1Items, { includeComboSignatures: false });
    const selectionFilter = await buildWorkoutSelectionFilterFromIds(selectedVariationIds);
    ...
    const equivalentRanked = await loadContextEquivalentExerciseRankedList({
        ...
        muscle,
        weightType,
        limit: GROUPED_BY_TYPE_RECOMMENDED.VARIATION_SEARCH_CANDIDATE_LIMIT,
        userId,
    });
    const tier2Docs = equivalentRanked.filter(
        (doc) => !isWorkoutExerciseSuggestionExcluded(doc, exclusionStateIdsOnly)
    );
    const tier2VariationIds = new Set(tier2Docs.map((doc) => doc?._id?.toString()).filter(Boolean));

    const tier3Docs = await getWorkoutDetailNameSearchExercises({
        search,
        userId,
        weightType,
        type,
        verified,
        muscle,
        exclusionState: exclusionStateIdsOnly,
        extraVariationIds: tier2VariationIds,
        candidateLimit: GROUPED_BY_TYPE_RECOMMENDED.VARIATION_SEARCH_CANDIDATE_LIMIT
    });

    const userObjectId = new mongoose.Types.ObjectId(userId);
    const [usageRows, exactUsageRows, equivalentPeerMap] = await Promise.all([
        aggregatePrimarySlotUsageByVariation(userObjectId),
        aggregateAllSlotUsageByVariation(userObjectId),
        loadExerciseEquivalentPeerMap()
    ]);
    ...
    const tier2Items = tier2Docs.map((variation) => ({
        kind: 'exercise',
        tier: 2,
        variation: injectUserUsageCountInVariations([variation], exactUsageMap, usageForScore)[0]
    }));
    const tier3Items = tier3Docs.map((variation) => ({
        kind: 'exercise',
        tier: 3,
        variation: injectUserUsageCountInVariations([variation], exactUsageMap, usageForScore)[0]
    }));

    const catalogComboSignatures = collectCatalogComboSignatures([...tier2Items, ...tier3Items]);
    const tier1ItemsRaw = dropPerformedCoveredByCatalogCombos(
        rankedTier1Items.slice(0, WORKOUT_DETAIL_TIER1_MAX),
        catalogComboSignatures
    );
    const tier1Items = await enrichPerformedItemsWithCardVariations(tier1ItemsRaw);

    let fullList = buildWorkoutSuggestionsOrderedList(tier1Items, tier2Items, tier3Items);
    fullList = filterWorkoutSuggestionsBySelection(fullList, selectionFilter);
    const total = fullList.length;
    const skip = (safePage - 1) * safeLimit;
    const suggestions = fullList.slice(skip, skip + safeLimit);
    ...
    return {
        ...
        meta: {
            ...
            tier2Total: tier2Items.length,
            tier3Total: tier3Items.length,
            maxDepthApplied,
            rootVariationId: familyPayload?.meta?.rootVariationId || String(contextVariationId)
        }
    };
}
...
module.exports = {
    ...
    getWorkoutDetailSuggestions,
    scorePerformedWorkoutSuggestionItem,
    rankPerformedWorkoutSuggestionItems,
    resolvePictureFromLoggedVariationIds,
    resolvePictureForVariationIds,
    getContextPrefixOverlapScore,
    getPerformedSuggestionNameScore,
    getPerformedSuggestionCountScore,
    tokenizeExact,
    getSearchableTokens,
};
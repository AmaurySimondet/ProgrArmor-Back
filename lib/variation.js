 ...
 */
const getVariationBySearch = async (search, type, sortBy, page, limit, verified, isExercice) => {
    const cacheKey = `variation_search_${normalizeString(search)}_${type}_${sortBy}_${page}_${limit}_${verified}_${isExercice}`;
 ...
 * @returns {Object} - The variations and the total number of variations
 */
const getAllVariations = async (type, sortBy, userId, page = 1, limit = 20, verified, isExercice) => {
    try {
        return await getOrSetCache('variations_all_' + type + '_' + sortBy + '_' + userId + '_' + page + '_' + limit + '_' + verified + '_' + isExercice, async () => {
            const query = {};
            if (type) {

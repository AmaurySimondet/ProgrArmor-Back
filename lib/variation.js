...
const getVariationBySearchBase = async (search, type, sortBy, page, limit, verified, isExercice, detailWeightType) => {
 ...
 */
const getVariationBySearch = async (search, type, sortBy, page, limit, verified, isExercice, myExercices, userId, detailWeightType) => {
    ...
    if (!shouldMergeMyExercises) {
        return getVariationBySearchBase(search, type, normalizedSortBy, page, limit, verified, isExercice, detailWeightType);
    }

    const candidateLimit = getRrfCandidateLimit(page, limit);
    const [regularSearch, myExercisesSearch] = await Promise.all([
        getVariationBySearchBase(search, type, normalizedSortBy, 1, candidateLimit, verified, isExercice, detailWeightType),
        set.getMyExercicesSearch(userId, search, 1, candidateLimit)
    ]);
    const filteredMyExercises = type
        ? myExercisesSearch.variations.filter(item => variationMatchesType(item, type))
        : myExercisesSearch.variations;
...
/**
 * Get variations grouped by type (types ordered by popularityScore).
...
const getVariationsGroupedByType = async (
    ...
    if (normalizedSortBy === 'popularity') {
        if (isExercice === false) {
    ...
    const [groups, totalTypesResult] = await Promise.all([
        Variation.aggregate([
            ...
            {
                $sort: normalizedSortBy === 'popularity' && isExercice === false

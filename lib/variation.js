...
const getVariationBySearch = async (search, type, sortBy, page, limit, verified, isExercice, myExercices, userId) => {
    const cacheKey = `variation_search_${normalizeString(search)}_${type}_${sortBy}_${page}_${limit}_${verified}_${isExercice}_${myExercices}_${userId}`;
...
const getAllVariations = async (type, sortBy, userId, page = 1, limit = 20, verified, isExercice, myExercices) => {
    try {
        return await getOrSetCache('variations_all_' + type + '_' + sortBy + '_' + userId + '_' + page + '_' + limit + '_' + verified + '_' + isExercice + '_' + myExercices, async () => {

...
const getVariationBySearchBase = async (search, type, sortBy, page, limit, verified, isExercice, detailWeightType, muscle, weightType) => {
    ...
    if (normalizedSortBy === 'recommended' && userId) {
        ...
        if (!shouldMergeMyExercises) {
            ...
            const [regularSearch, myExercisesSearch] = await Promise.all([
                getVariationBySearchBase(search, type, normalizedSortBy, 1, candidateLimit, verified, isExercice, detailWeightType, muscle, weightType),
...
async function loadContextEquivalentExerciseRankedList({ contextVariationId, type, verified, muscle, weightType, limit }) {

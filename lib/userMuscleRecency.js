...
function aggregateMuscleRecencyFromSets(
    ...
    for (const set of sets) {
        ...
        for (const muscleKey of muscleKeysForSet) {
            const current = muscleLastDate[muscleKey];
            if (!current || set.date > current) {
                muscleLastDate[muscleKey] = set.date;
            }
        }
    }

    const muscles = {};
    for (const [muscleKey, lastWorkedDate] of Object.entries(muscleLastDate)) {
        muscles[muscleKey] = {
            lastWorkedDate: lastWorkedDate.toISOString().slice(0, 10),
            daysSince: computeDaysSince(lastWorkedDate, referenceDate),
        };
    }
...
async function computeUserMuscleRecency(userIdObj, referenceDate = new Date()) {
    const sets = await SeanceSet.find({ user: userIdObj })
        .select('date variations.variation')
        .lean();

...
const { computeBestWeekStreak, computeCurrentWeekStreak, getWeekIndex } = require('../utils/weekStreak');
const { personalRecordFlagMongoMatch } = require('../utils/personalRecordFlags');
const { seanceTotalWeightKgMongoSum } = require('../utils/seanceVolume');
...
async function modifyUser(req, res) {
    try {
        ...
        await backfillSeanceSetsForUser(userId, { fullRefresh: true });
...
async function userStats(req, res) {
    try {
        ...
        const [
            ...
            // Same semantics as success totalPrCount: one counted set (PR or ATH) = one record
            ...
            // Same semantics as success sum_volume_total: sum seance.stats.totalWeight
            Seance.aggregate([
                { $match: { user: userIdObj } },
                { $group: { _id: null, totalKg: seanceTotalWeightKgMongoSum() } },
            ]),
        ...
        const totalVolumeLbs = totalVolume * KG_TO_LB;
...
/**
 * Returns users from the current user's following list, ranked by session count.
 * @param {Request} req - The request object.
 * @param {Response} res - The response object.
 * @returns {Promise<void>} - A promise that resolves to void.
 */
async function getFollowingRecommended(req, res) {
    try {
        if (!ensureAuthenticated(req, res)) {
            return;
        }
        ...
        console.error('[Following Recommended] Error:', error);
        return res.status(500).json({
            success: false,
            message: "Error fetching following recommendations"
        });
    }
}
...
/**
 * Calculates and returns a user's workout regularity score (optionally scoped by date range).
 * @param {Request} req - The request object (query: userId, dateMin?, dateMax?)
 * @param {Response} res - The response object
 * @returns {Promise<void>} - A promise that resolves to void
 */
async function getRegularityScore(req, res) {
    try {
        const { userId, dateMin, dateMax } = req.query;
        if (!userId) {
            throw new Error("User ID is required");
        }
        ...
        const score = await (async () => {
            ...
            const seanceQuery = { user: userId, ...(hasDateFilter ? { date: dateFilter } : {}) };
            const seances = await Seance.find(seanceQuery)
                .sort({ date: 1 })
                .select('date name _id program')
                .populate('program', 'name initials color folder isArchived')
                .lean();

            const allSeancesForStreaks = hasDateFilter
                ? await Seance.find({ user: userId }).sort({ date: 1 }).select('date').lean()
                : seances;

            const currentStreak = computeCurrentWeekStreak(allSeancesForStreaks);
            const bestStreak = computeBestWeekStreak(allSeancesForStreaks);

            if (seances.length === 0) {
                return { average: 0, currentStreak, bestStreak, uniqueWeeks: 0, totalWeeksSinceFirstWorkout: 0, seances: [] };
            }

            const now = new Date();
            const rangeEnd = dateMax ? new Date(dateMax) : now;
            if (!Number.isFinite(rangeEnd.getTime())) {
                rangeEnd.setTime(now.getTime());
            }
            rangeEnd.setHours(23, 59, 59, 999);

            const rangeStart = dateMin ? new Date(dateMin) : new Date(seances[0].date);
            if (!Number.isFinite(rangeStart.getTime())) {
                rangeStart.setTime(new Date(seances[0].date).getTime());
            }
            rangeStart.setHours(0, 0, 0, 0);

            const endWeekIndex = getWeekIndex(rangeEnd);
            const firstWeekIndex = getWeekIndex(rangeStart);
            const totalWeeksSinceFirstWorkout = Math.max(1, endWeekIndex - firstWeekIndex + 1);
            ...
            const uniqueWeeks = [...new Set(weekIndices.filter(Number.isFinite))].sort((a, b) => a - b);
...
async function createUserMeasure(req, res) {
    try {
        ...
        const measures = await UserMeasure.find({ userId }).sort({ measuredAt: 1 }).lean();
        const dateRanges = getAffectedDateRangesForMeasureChange({
            measures,
            newMeasuredAt: measure.measuredAt,
        });
        await backfillSeanceSetsForUser(userId, { dateRanges, fullRefresh: false });
...
async function updateUserMeasure(req, res) {
    try {
        ...
        const oldMeasuredAt = measure.measuredAt;
        const weightChanged = weight !== undefined;
        const measuredAtChanged = measuredAt !== undefined;
        ...
        if (weightChanged || measuredAtChanged) {
            const measures = await UserMeasure.find({ userId }).sort({ measuredAt: 1 }).lean();
            const dateRanges = getAffectedDateRangesForMeasureChange({
                measures,
                newMeasuredAt: measure.measuredAt,
                oldMeasuredAt: measuredAtChanged ? oldMeasuredAt : undefined,
            });
            await backfillSeanceSetsForUser(userId, { dateRanges, fullRefresh: false });
        }
...
async function deleteUserMeasure(req, res) {
    try {
        ...
        const measures = await UserMeasure.find({ userId }).sort({ measuredAt: 1 }).lean();
        const deletedWindow = windowAfterDeletedMeasure(measures, deleted.measuredAt);
        if (deletedWindow) {
            await backfillSeanceSetsForUser(userId, {
                dateRanges: [deletedWindow],
                fullRefresh: false,
            });
        }

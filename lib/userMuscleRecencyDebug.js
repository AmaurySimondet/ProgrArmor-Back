...
function resolveContributionSource(targets, reverseEquivalentMap) {
    if (targets.length === 0) return 'none';
    if (targets.every(isMuscleTypeDetail)) return 'muscle-detail';
    const target = targets[0];
    const direct = extractMuscleKeysFromVariation(target);
    if (direct.all.length > 0) return 'primary';
    if (reverseEquivalentMap?.get(String(target._id))?.length) return 'primary-equivalentTo';
    return 'primary';
}

function traceSetMuscleContribution(set, variationById, reverseEquivalentMap = new Map()) {
    const resolvedChain = resolveVariationChainFromSet(set, variationById);
    const targets = getMuscleResolutionTargets(resolvedChain);
    const source = resolveContributionSource(targets, reverseEquivalentMap);

    const chain = resolvedChain.map((variation) => {
        const direct = extractMuscleKeysFromVariation(variation);
        const isTarget = targets.some((target) => String(target._id) === String(variation._id));
        return {
            variationId: String(variation._id),
            name: variation.name?.fr || variation.name?.en || null,
            isExercice: variation.isExercice === true,
            isMuscleTypeDetail: isMuscleTypeDetail(variation),
            muscles: variation.muscles || null,
            muscleKeys: isTarget ? (
                direct.all.length > 0
                    ? direct.all
                    : (reverseEquivalentMap.get(String(variation._id)) || [])
            ) : [],
            source: isTarget ? source : (isMuscleTypeDetail(variation) ? 'muscle-detail-unused' : 'ignored-detail'),
        };
    });

    const muscleKeys = resolveMuscleKeysForSet(set, variationById, reverseEquivalentMap);
    const primaryExercise = getPrimaryExerciseVariationFromList(resolvedChain);
...
function computeSessionStyleMusclesForSets(sets = [], variationById, reverseEquivalentMap = new Map()) {
    ...
    for (const [variationKey, groupSets] of groups.entries()) {
        ...
        const resolved = resolveMusclesFromVariationChain(resolvedVariations, reverseEquivalentMap);

        for (const key of resolved.primary) primary.add(key);
        for (const key of resolved.secondary) secondary.add(key);

...
const { computeCurrentWeekStreak } = require("../utils/weekStreak");
const {
    isCountedPersonalRecordFlag,
    countPersonalRecordSets,
} = require("../utils/personalRecordFlags");
const { sumSeanceTotalWeightKg } = require("../utils/seanceVolume");
const { success: { SUCCESS_ALL_MAX_LIMIT } } = require('../constants');
...
function getSetRepsEquivalent(setDoc) {
    if (setDoc?.unit === 'cardio') return 0;
...
/**
 * Séance avec au moins `minSets` séries, chacune marquée PR ou ATH (même logique que `totalPrCount`).
...
function seanceHasOnlyPrSets(seanceSets, minSets = 1) {
    ...
    return seanceSets.every(s => isCountedPersonalRecordFlag(s.PR));
}
...
function evaluateSuccessWithDetail(condition, context) {
    ...
    if (code === "user_id") {
        const target = toObjectId(condition.userId);
        const uid = toObjectId(userId);
        const ok = Boolean(target && uid && target.equals(uid));
 ...
 */
/**
 * La définition du succès a changé depuis la dernière validation du déblocage
 * (Success.conditionUpdatedAt plus récent que le repère côté UserSuccess).
 */
function isDefinitionOutOfSyncWithUnlock(userSuccess, successDoc) {
    const condAt = successDoc?.conditionUpdatedAt;
    if (!condAt) return false;
    const synced = userSuccess?.conditionSyncedAt;
    const ref = synced != null ? new Date(synced) : new Date(userSuccess?.createdAt || 0);
    if (!Number.isFinite(ref.getTime())) return false;
    return new Date(condAt) > ref;
}

function enrichUserSuccessWithSyncFlag(us) {
    if (!us) return us;
    return {
        ...us,
        definitionChangedAfterUnlock: isDefinitionOutOfSyncWithUnlock(us, us.success),
    };
}
...
function compareSuccessCatalogDefault(a, b) {
    const levelCmp = (b.level || 0) - (a.level || 0);
...
function sortSuccessCatalog(items = [], { sortBy = "obtainedAt", sortOrder = "desc" } = {}) {
    if (sortBy === "obtainedAt") {
        const dir = sortOrder === "asc" ? 1 : -1;
        return [...items].sort((a, b) => {
            const aOwned = a.isOwned && a.obtainedAt;
            const bOwned = b.isOwned && b.obtainedAt;
            if (!aOwned && !bOwned) return compareSuccessCatalogDefault(a, b);
            if (!aOwned) return 1;
            if (!bOwned) return -1;
            const da = new Date(a.obtainedAt).getTime();
            const db = new Date(b.obtainedAt).getTime();
            if (da !== db) return (da - db) * dir;
            return compareSuccessCatalogDefault(a, b);
        });
    }
    return [...items].sort(compareSuccessCatalogDefault);
}
...
async function processNewSuccesses(userId, ttlMinutes = 5) {
    ...
    if (lastAt && now - lastAt < ttlMs) {
        ...
        const enriched = list.map(enrichUserSuccessWithSyncFlag);
        return { userSuccesses: sortUserSuccessesBySuccessLevelDesc(enriched), recalculated: false };
    }
    ...
    const [successes, existingUserSuccesses, seances, sets, allVariations, imageCount] = await Promise.all([
        ...
        Seanceset.find({ user: userObjectId })
            .select(
                "_id seance date unit value weightLoad elastic PR variations brzycki epley oneRepMaxIncludesBodyweight oneRepMaxUserWeightKg oneRepMaxExerciseBodyWeightRatio brzyckiWithBodyweight epleyWithBodyweight effectiveWeightLoad effectiveWeightLoadWithBodyweight weightLoadLbs effectiveWeightLoadLbs effectiveWeightLoadWithBodyweightLbs"
            )
            .lean(),
    ...
    const successById = new Map(successes.map(s => [String(s._id), s]));
    ...
    const context = {
        ...
        totalPrCount: countPersonalRecordSets(sets),
        totalVolume: sumSeanceTotalWeightKg(seances),
        currentWeekStreak: computeCurrentWeekStreak(seances),
        imageCount,
    };

    for (const us of existingUserSuccesses) {
        const successDoc = successById.get(String(us.success));
        if (!successDoc || !isDefinitionOutOfSyncWithUnlock(us, successDoc)) continue;
        const { ok, unlockDetail } = evaluateSuccessWithDetail(successDoc.condition || {}, context);
        if (!ok) {
            await UserSuccess.deleteOne({ _id: us._id, user: userObjectId });
            existingBySuccessId.delete(String(us.success));
        } else {
            const setDoc = {};
            if (unlockDetail && Object.keys(unlockDetail).length > 0) {
                setDoc.unlockDetail = unlockDetail;
            }
            if (successDoc.conditionUpdatedAt) {
                setDoc.conditionSyncedAt = successDoc.conditionUpdatedAt;
            }
            if (Object.keys(setDoc).length > 0) {
                await UserSuccess.updateOne({ _id: us._id, user: userObjectId }, { $set: setDoc });
            }
        }
    }
    ...
    for (const success of unownedSuccesses) {
        ...
        if (ok) {
            ...
            if (success.conditionUpdatedAt) {
                doc.conditionSyncedAt = success.conditionUpdatedAt;
            }
            toCreate.push(doc);
        }
    }
    ...
    const unackList = await UserSuccess.find({ user: userObjectId, acknowledged: false })
        .populate("success")
        .lean();
    const enrichedUnack = unackList.map(enrichUserSuccessWithSyncFlag);
    return {
        userSuccesses: sortUserSuccessesBySuccessLevelDesc(enrichedUnack),
        recalculated: true,
    };
}
...
async function getAllSuccessesForUser(userId, options = {}) {
    ...
    const sortBy = options.sortBy === "level" ? "level" : "obtainedAt";
    const sortOrder = options.sortOrder === "asc" ? "asc" : "desc";
    const sortByObtainedAt = sortBy === "obtainedAt";
    ...
    let successesQuery = Success.find({}).lean();
    if (!sortByObtainedAt) {
        successesQuery = successesQuery.sort({ level: -1, type: 1 });
        if (limit !== null) {
            successesQuery = successesQuery.skip((page - 1) * limit).limit(limit);
        }
    }
    ...
    let successesMapped = successes.map(success => {
        ...
        return {
            ...
            obtainedAt: us?.createdAt ? new Date(us.createdAt).toISOString() : null,
            unlockDetail: us?.unlockDetail ?? null,
            definitionChangedAfterUnlock: us ? isDefinitionOutOfSyncWithUnlock(us, success) : false,
            usersTotalForProportion: totalActive,
            usersWithSuccessProportion: totalActive > 0 ? owners / totalActive : 0,
        };
    });

    successesMapped = sortSuccessCatalog(successesMapped, { sortBy, sortOrder });

    if (limit !== null && sortByObtainedAt) {
        const start = (page - 1) * limit;
        successesMapped = successesMapped.slice(start, start + limit);
    }
    ...
    const successesMapped = list
        .map(us => {
            ...
            return {
                ...
                usedOnProfile: true,
                unlockDetail: us?.unlockDetail ?? null,
                definitionChangedAfterUnlock: isDefinitionOutOfSyncWithUnlock(us, s),
                usersTotalForProportion: totalActive,
                usersWithSuccessProportion: totalActive > 0 ? owners / totalActive : 0,
            };
        })
...
module.exports = {
    ...
    isDefinitionOutOfSyncWithUnlock,
};

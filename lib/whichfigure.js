...
function computeTargetValueFromOneRm(oneRmKg, targetUnit, effectiveWeightLoadRaw, weightedBodyweightKg = 0) {
    ...
    if (Number.isFinite(rBrzycki) && rBrzycki < MAX_BRZYCKI_TARGET_REPS) candidates.push(rBrzycki);
...
function collectBestPeakFromDetailedPrs(prs) {
    ...
    for (const key of keys) {
        for (const unitKey of ['repetitions', 'seconds']) {
            ...
            const normalizedOneRmForRecommendationRaw = Number(set?.normalizedOneRmForRecommendation);
            const normalizedOneRmForRecommendation = Number.isFinite(normalizedOneRmForRecommendationRaw)
                && normalizedOneRmForRecommendationRaw > 0
                ? normalizedOneRmForRecommendationRaw
                : normalizedOneRm;
...
async function computeRecommendedWeightFigure({
    ...
    const recommendations = await Promise.all(payload.entries.map(async (entry) => {
        const graphDifficultyScore = difficultyScoreByTarget.get(String(entry.variationId));
        ...
        const weightedBodyweightKg = policy.includeBodyweight && Number.isFinite(Number(userWeightKg))
            ...
            : 0;
        const oneRmForRecommendation = Number(
            peak.normalizedOneRmForRecommendation ?? peak.normalizedOneRm
        );

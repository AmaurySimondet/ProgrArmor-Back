...
function computeTargetLoadFromOneRm(oneRmKg, targetUnit, targetValueRaw, weightedBodyweightKg = 0) {
    ...
    const denom = 1 + r / 30;
    if (denom > 0) {
        // oneRmKg in this flow is an external-load equivalent (same semantics as graph normalizedOneRm),
        // so we do not subtract bodyweight again here.
...
function buildFigureTargetLoadInverseBreakdown(oneRmKg, targetUnit, targetValueRaw, weightedBodyweightKg = 0) {
    ...
    if (Number.isFinite(r) && Number.isFinite(oneRm) && oneRm > 0) {
        const denom = 1 + r / 30;
        if (denom > 0) wEpley = oneRm / denom;
        if (r <= MAX_BRZYCKI_TARGET_REPS && r < 37) {
            const factor = (37 - r) / 36;
            wBrzycki = oneRm * factor;
        }
    }
    ...
    return {
        ...
        weightedBodyweightKgNotSubtracted: b,
        wEpleyFromChargeUtileOneRm: Number.isFinite(wEpley) ? Math.round(wEpley * 1000) / 1000 : null,
        wBrzyckiFromChargeUtileOneRm: Number.isFinite(wBrzycki) ? Math.round(wBrzycki * 1000) / 1000 : null,
        ...
        hypotheticalExternalIfSubtractBw: avg != null && b > 0
            ? Math.round((avg - b) * 1000) / 1000
            : null,
        note: 'Figure path: normalizedOneRm charge utile ; inverse NE soustrait PAS le PDC (charge externe directe).',
    };
}
...
async function augmentDirectEntryPeakWithSessionSets({
    ...
    if (!peak || !Number.isFinite(sessionOneRm) || sessionOneRm <= 0) {
        return { peak, usedSessionSets };
    }
    const prOneRm = Number(peak?.normalizedOneRm);
    if (Number.isFinite(prOneRm) && sessionOneRm <= prOneRm) {
        return { peak, usedSessionSets };
    }
    return {
        peak: {
...
function enrichDirectComboEntryNames(entries, effectiveReferenceSignature, contextualMergeMeta) {
    if (!effectiveReferenceSignature?.includes('|')) return entries;
    ...
    for (const entry of entries) {
        ...
        for (const part of parseProgressionSignatureToObjectIds(key)) {
            if (!nameById.has(part)) nameById.set(part, entry.name);
        }
    }
    ...
    return entries.map((entry) => {
        ...
        const comboName = buildComboDisplayNameFromSignature(key, nameById, isExerciseByKey);
...
async function computeRecommendedWeightFigure({
    ...
    const recommendations = await Promise.all(payload.entries.map(async (entry) => {
        ...
        const weightedBodyweightKg = policy.includeBodyweight && Number.isFinite(Number(userWeightKg))
            ...
            : 0;
        const oneRmForRecommendation = Number(peak.normalizedOneRm);

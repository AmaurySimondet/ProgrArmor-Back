...
const { compareAndAssignPR, getEffectiveLoad } = require('../utils/set');
...
async function fetchSetsForPR(userId, exercice, categories, dateMin, variations) {
...
async function isPersonalRecord(userId, seanceId, unit, value, weightLoad, elastic, variations) {
    ...
    const currentEffectiveLoad = getEffectiveLoad({ weightLoad, elastic });
    ...
    try {
        ...
        let sets = allSets.filter(s => {
            ...
            // Compare effective load (weightLoad adjusted by elastic use/tension)
            if (s.weightLoad == null || getEffectiveLoad(s) < currentEffectiveLoad) isBetterOrEqual = false;
        ...
        if (sets.length === 0) {
            ...
            const bestSet = sets.reduce((best, current) => {
                ...
                if (current.value === best.value && getEffectiveLoad(current) > getEffectiveLoad(best)) return current;
                return best;
            });
            ...
            if (getEffectiveLoad(bestSet) > currentEffectiveLoad) isBestSet = false;
...
module.exports = { getSets, getTopExercices, createSet, getPRs, getDetailedPRs, getLastFormats, deleteSets, isPersonalRecord, getMyExercicesSearch, getMyExercicesAll, getPersonalRecordsSummary };

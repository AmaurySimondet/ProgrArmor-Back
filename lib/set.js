 ...
 * - belowHistoricalLoadAtSameReps: +0 reps/sec et charge négative
...
function resolvePrDetailDeltas({
    ...
    } else if (statusReason === 'sameLoadAtSameReps') {
        kgDelta = 0;
        lbsDelta = 0;
    } else if (statusReason === 'belowHistoricalLoadAtSameReps') {
        setZeroValueDelta();
        setLoadDeltaFromReference();
    }

    // Garde-fou: on ne force pas de deltas pour les états sans raison connue.
    if (!status && (statusReason == null || statusReason === 'noHistoricalSameRepsOrSecs')) {
        return { repsDelta, secondsDelta, kgDelta, lbsDelta };
    }

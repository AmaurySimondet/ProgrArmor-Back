...
async function getSets(userId, excludedSeanceId, seanceId, exercice, categories, unit, value, weightLoad, elasticTension, dateMin, dateMax, fields, variations) {
    try {
        const cacheKey = `sets_${userId || ''}_${excludedSeanceId || ''}_${seanceId || ''}_${exercice || ''}_${categories || ''}_${unit || ''}_${value || ''}_${weightLoad || ''}_${elasticTension || ''}_${dateMin || ''}_${dateMax || ''}_${fields || ''}_${variations || ''}`;

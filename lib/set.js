...
const { invalidateSetCaches, getOrSetCache, invalidateSeanceCaches } = require('../controllers/utils/cache');
 ...
 */
async function getSets(userId, seanceId, exercice, categories, unit, value, weightLoad, elastic, dateMin, dateMax, fields) {
    try {
        const cacheKey = `sets_${userId || ''}_${seanceId || ''}_${exercice || ''}_${categories || ''}_${unit || ''}_${value || ''}_${weightLoad || ''}_${elastic || ''}_${dateMin || ''}_${dateMax || ''}_${fields || ''}`;

...
async function getNormalFlowPerformedVariationFamilies({
    ...
    if (inputVariationIds.length === 1) {
        ...
        const equivalentToPath = Array.isArray(rootVariationDoc?.equivalentTo)
            ...
            : [];
        familySeedIds = equivalentToPath.length > 0
            ? equivalentToPath
            : [rootVariationIdStr];
    } else {
    ...
    if (familySeedIds.length > 0) {
        const maxDepthForPath = Math.min(maxDepthApplied, familySeedIds.length);
        for (let depth = 1; depth <= maxDepthForPath; depth += 1) {
            familyPrefixes.push(familySeedIds.slice(0, depth));
        }
    } else {
        familyPrefixes.push([rootExerciseId]);
    }

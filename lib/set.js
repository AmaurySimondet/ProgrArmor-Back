...
const { computeStrengthPeakFromFigurePoints } = require('./strengthPeak');
const {
...
/**
 * Contexte du graphe de progression : certaines arêtes sont stockées avec
 * `contextVariationId` = variation de référence (ex. L-Sit) alors que l’UI
 * envoie l’exercice principal (ex. V-Sit via `progressionReferenceVariationId`).
 * Retourne cette référence si définie, sinon l’id principal normalisé.
 */
...
async function resolveReferenceVariationIdsForProgression({
    ...
    if (normalizedMainExerciseId && mongoose.Types.ObjectId.isValid(normalizedMainExerciseId)) {
        const mainDoc = await Variation.findById(
            String(normalizedMainExerciseId),
            { progressionReferenceVariationId: 1 }
        ).lean();
        const progressionRefId = mainDoc?.progressionReferenceVariationId
            ? String(mainDoc.progressionReferenceVariationId)
            : null;
        if (progressionRefId && mongoose.Types.ObjectId.isValid(progressionRefId)) {
            return [progressionRefId];
        }
    }
...
async function augmentSetsWithNormalizedMetrics({
    ...
    for (const setDoc of sets) {
        ...
        const normalizedBrzycki = peakOneRmEstimates.brzycki != null
            ? peakOneRmEstimates.brzycki
            : (Number.isFinite(normalizedExternalLoad)
                && Number.isFinite(Number(repsEquivalent))
                && Number(repsEquivalent) > 0
                && shouldUseBrzyckiForRepsEquivalent(Number(repsEquivalent))
                ? toRoundedOrNull(estimateOneRepMaxBrzycki(normalizedExternalLoad, Number(repsEquivalent)))
                : null);
        const normalizedEpley = peakOneRmEstimates.epley != null
            ? peakOneRmEstimates.epley
            : (Number.isFinite(normalizedExternalLoad) && Number.isFinite(Number(repsEquivalent)) && Number(repsEquivalent) > 0
                ? toRoundedOrNull(estimateOneRepMaxEpley(normalizedExternalLoad, Number(repsEquivalent)))
                : null);
        ...
        let normalizedOneRmForRecommendation = normalizedOneRm;
        if (includeBodyweight && weightedBodyweightKg > 0) {
            const brzyckiBw = Number(set.brzyckiWithBodyweight ?? set.brzycki_with_bodyweight);
            const epleyBw = Number(set.epleyWithBodyweight ?? set.epley_with_bodyweight);
            const normBrzyckiBw = Number.isFinite(brzyckiBw) && brzyckiBw > 0
                ? toRoundedOrNull(brzyckiBw * difficultyFactor)
                : null;
            const normEpleyBw = Number.isFinite(epleyBw) && epleyBw > 0
                ? toRoundedOrNull(epleyBw * difficultyFactor)
                : null;
            if (normBrzyckiBw != null && normEpleyBw != null) {
                normalizedOneRmForRecommendation = toRoundedOrNull((normBrzyckiBw + normEpleyBw) / 2);
            } else if (normBrzyckiBw != null || normEpleyBw != null) {
                normalizedOneRmForRecommendation = normBrzyckiBw ?? normEpleyBw;
            } else if (normalizedBrzycki != null) {
                normalizedOneRmForRecommendation = toRoundedOrNull(Number(normalizedBrzycki) + weightedBodyweightKg);
            }
        }
        ...
        augmentedSets.push({
            ...
            repsEquivalent: Number.isFinite(Number(repsEquivalent)) ? Number(repsEquivalent) : null,
            brzycki: peakOneRmEstimates.brzycki,
            epley: peakOneRmEstimates.epley,
...
async function getNormalizedProgressionTimeseries({
    ...
    for (const setDoc of filteredSets) {
        ...
        const effectiveLoadWithBodyweight = getEffectiveLoadKg(set, {
    ...
    const strengthPeak = computeStrengthPeakFromFigurePoints(points, { weightUnit });
...
async function getFigureDetailedPRs({
    ...
    for (const variationId of targetIds) {
        ...
        const augmented = await augmentSetsWithNormalizedMetrics({
            sets: setsForAugment,
            userId,
            // Conserver une normalisation par cible pour le calcul de perf/reco;
            // le score de difficulté affiché est géré séparément dans whichfigure.

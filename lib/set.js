 ...
 * @returns {string} "PR" | "SB" | "NB" | "ATH" | null — PR/SB/NB = valeur vs pic historique à charge externe égale ; ATH = 1RM estimé bat le pic global
 */
...
async function evaluatePersonalRecordWithContext(
    ...
    try {
        ...
        } else if (setsAtSameLoad.length === 0) {
            // Première perf à cette charge externe (historique DB uniquement ; sessionPeerSets servent à l'ATH).
            status = "NB";
            statusReason = 'noHistoricalAtSameLoad';
        } else if (maxValueAtSameLoad != null && Number.isFinite(currentValue)) {
 ...
 * - belowHistoricalValueAtSameLoad: reps/sec négatif et +0 kg/lbs
 */
function resolvePrDetailDeltas({
    ...
    // Garde-fou: on ne force pas de deltas pour les états sans raison connue.
    if (!status && (statusReason == null || statusReason === 'noHistoricalAtSameLoad')) {
        return { repsDelta, secondsDelta, kgDelta, lbsDelta };
    }

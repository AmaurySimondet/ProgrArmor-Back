...
const {
    ...
    resolveLateralModeForRecommendation,
...
const MY_EXERCISE_GROUP_SORT_STAGE = {
    $sort: { count: -1, _id: 1 },
};

const MY_EXERCISE_NORMALIZE_VARIATIONS_STAGE = {
    $set: {
        sortedVariationIds: { $sortArray: { input: '$variations.variation', sortBy: 1 } },
    },
};

const MY_EXERCISE_GROUP_BY_VARIATIONS_STAGE = {
    $group: {
        _id: '$sortedVariationIds',
        count: { $sum: 1 },
    },
};

async function aggregateMyExerciceGroups(userId) {
    const db = mongoose.connection?.db;
    if (!db) {
        throw new Error('MongoDB connection is not ready');
    }

    return db.collection('seancesets').aggregate([
        { $match: { user: new mongoose.Types.ObjectId(userId) } },
        MY_EXERCISE_NORMALIZE_VARIATIONS_STAGE,
        MY_EXERCISE_GROUP_BY_VARIATIONS_STAGE,
        MY_EXERCISE_GROUP_SORT_STAGE,
    ]).toArray();
}

async function hydrateExerciseGroups(rawGroups = []) {
    if (!rawGroups.length) {
        return [];
    }

    const idsListofLists = rawGroups.map((variation) => variation._id.map((id) => id.toString()));
    ...
    return rawGroups.map((variation) => {
        ...
        for (const id of ids) {
            const variationDoc = variationsDocs.find((doc) => doc._id.toString() === id.toString());
...
function finalizeExerciseGroups(hydratedGroups = []) {
    const merged = normalizeAndMergeExerciseGroups(hydratedGroups);
    return sortExerciseGroupsByCountStable(merged);
}

function paginateExerciseGroups(groups, page, limit, fetchAll = false) {
    if (fetchAll) {
        return groups;
    }
    ...
    return groups.slice(start, start + safeLimit);
}
...
async function getMyExercicesSearch(userId, search, page, limit) {
    try {
        ...
        const rawGroups = await db.collection('seancesets').aggregate(
            searchPipeline,
            { maxTimeMS: SEARCH_MAX_TIME_MS },
        ).toArray();

        const hydratedGroups = await hydrateExerciseGroups(rawGroups);
        const finalizedGroups = finalizeExerciseGroups(hydratedGroups);
        const total = finalizedGroups.length;
        const variations = paginateExerciseGroups(finalizedGroups, page, limit);
 ...
 * @returns {Promise<Array<Object>>} - A promise that resolves to an array of exercices.
 */
async function getMyExercicesAll(userId, page, limit, options = {}) {
    try {
        const rawGroups = await aggregateMyExerciceGroups(userId);
        const hydratedGroups = await hydrateExerciseGroups(rawGroups);
        const finalizedGroups = finalizeExerciseGroups(hydratedGroups);
        const total = finalizedGroups.length;
        const variations = paginateExerciseGroups(
            finalizedGroups,
            page,
            limit,
...
async function getProgressionPRs({
    ...
    const normalizedLateralMode = resolveLateralModeForRecommendation(lateralMode, unilateralSide);
    const referenceIds = await resolveReferenceVariationIdsForProgression({
        referenceVariations,
        mainExerciseId
    });
...
async function resolveSharedFamilyPrFetchForDetailedPrs({
    userId,
    excludedSeanceId = null,
    exercice = null,
    categories = null,
    ...
    const { rawSets, familyScope } = await resolveProgressionFamilyScopedRawSetsForPrs({
        ...
        includedVariationIds,
        excludedVariationSignatures,
        logMode: 'figure-detailed-entries-shared',
    });
    return {
        referenceIds,
        normalizedMainExerciseId,
...
async function getProgressionDetailedPRs({
    ...
    includeAllGraphTargets = false,
    maxTargets = 40,
    preloadedFamilyPrFetch = null,
}) {
    const normalizedLateralMode = preloadedFamilyPrFetch?.normalizedLateralMode
        ?? resolveLateralModeForRecommendation(lateralMode, unilateralSide);
    const referenceIds = preloadedFamilyPrFetch?.referenceIds
        ?? await resolveReferenceVariationIdsForProgression({
            referenceVariations,
            mainExerciseId,
        });
    const referenceCanonicalId = await resolveCanonicalVariationIdFromIds(referenceIds);
    ...
    let normalizedMainExerciseId = preloadedFamilyPrFetch?.normalizedMainExerciseId
        ?? (mongoose.Types.ObjectId.isValid(mainExerciseId) ? String(mainExerciseId) : null);
    if (!normalizedMainExerciseId) {
        throw new Error('mainExerciseId est requis');
    }
    if (!preloadedFamilyPrFetch) {
        normalizedMainExerciseId = await resolveMainExerciseIdForProgression(normalizedMainExerciseId);
    }
    const progressionGraphContextVariationId = await resolveGraphContextVariationId(normalizedMainExerciseId);

    const { rawSets, familyScope } = preloadedFamilyPrFetch
        ? {
            rawSets: preloadedFamilyPrFetch.rawSets,
            familyScope: preloadedFamilyPrFetch.familyScope,
        }
        : await resolveProgressionFamilyScopedRawSetsForPrs({
            userId,
            exercice,
            categories,
            dateMin,
 ...
 */
function computeOverallBestFromPrs(prs) {
...
function computePersonalRecordPrsFromSets(sets = []) {
    const prs = {
        Puissance: { repetitions: null, seconds: null },
        Force: { repetitions: null, seconds: null },
        Volume: { repetitions: null, seconds: null },
    ...
    prs.Best = computeOverallBestFromPrs(prs);
    return prs;
}

async function buildVariationGroupsByExerciseSignature(exercises = []) {
    const uniqueExercisesBySignature = new Map();
    for (const exercice of exercises) {
        const signature = getVariationSignature(exercice._id);
        if (!uniqueExercisesBySignature.has(signature)) {
            uniqueExercisesBySignature.set(signature, exercice);
        }
    }

    const entries = await Promise.all(
        [...uniqueExercisesBySignature.entries()].map(async ([signature, exercice]) => {
            const groups = await getAlternativeVariationGroups(exercice._id);
...
function buildPersonalRecordSummaryFromExercise(exercice, sets, prs) {
    return {
        variations: exercice.variations,
        variationIds: exercice._id,
        usageCount: exercice.count,
        prs,
        totalSets: sets.length,
    };
}

async function computePersonalRecordSummaryForExercise(userId, exercice, dateMin = null) {
    const variationIds = exercice._id;
    ...
    const sets = await Set.find(query).sort({ date: 1 }).lean().exec();
    const prs = computePersonalRecordPrsFromSets(sets);
    return buildPersonalRecordSummaryFromExercise(exercice, sets, prs);
}
...
async function getPersonalRecordsSummary(userId, page = 1, limit = 10, dateMin = null) {
    try {
        ...
        if (dateMin) {
            setsQuery.date = { $gte: new Date(dateMin) };
        }

        const [{ variations: allExercises }, allSets] = await Promise.all([
            getMyExercicesAll(userId, 1, limit, { fetchAll: true }),
            Set.find(setsQuery).sort({ date: 1 }).lean().exec(),
        ]);

        if (!allExercises.length) {
            return { summaries: [], total: 0, page: safePage, limit: safeLimit };
        }

        const variationGroupsBySignature = await buildVariationGroupsByExerciseSignature(allExercises);

        const setsBySignature = indexSetsByVariationSignature(allSets);

        const summaries = allExercises.map((exercice) => {
            const signature = getVariationSignature(exercice._id);
            const variationGroups = variationGroupsBySignature.get(signature) || [];
            const sets = collectSetsMatchingVariationGroups(setsBySignature, variationGroups);
            const prs = computePersonalRecordPrsFromSets(sets);
            return buildPersonalRecordSummaryFromExercise(exercice, sets, prs);
        });

        const summariesWithPr = summaries.filter((summary) => summary.prs?.Best != null);
        const sortedSummaries = sortPersonalRecordSummaries(summariesWithPr);
        const total = sortedSummaries.length;
        const start = (safePage - 1) * safeLimit;
        const paginatedSummaries = sortedSummaries.slice(start, start + safeLimit);
 ...
 * @returns {string} "PR" | "SB" | "NB" | "ATH" | null — bi-axial : valeur à charge L et charge à valeur V ; ATH = 1RM estimé bat le pic global
 */
...
/** Logs isPr / ATH : opt-in via DEBUG_PR_ISPR=1 uniquement. */
function isPrDebugLoggingEnabled() {
    return process.env.DEBUG_PR_ISPR === '1' || process.env.DEBUG_PR_ISPR === 'true';
}
...
async function fetchPersonalRecordCandidateSets(
    ...
    isUnilateral = undefined,
    unilateralSide = undefined,
    historicalBeforeDate = undefined,
) {
    const query = {};
    if (userId) {
        query.user = new mongoose.Types.ObjectId(userId);
    ...
    if (historicalBeforeDate != null) {
        const cutoff = new Date(historicalBeforeDate);
        if (!Number.isNaN(cutoff.getTime())) {
            query.date = { $lt: cutoff };
        }
    }
    if (unit) {
        query.unit = unit;
...
function getPrContextKey({ unit, isUnilateral, unilateralSide } = {}) {
    return [
        unit ?? '',
        isUnilateral === true ? '1' : (isUnilateral === false ? '0' : ''),
        unilateralSide ?? '',
    ].join('|');
}

async function loadPrEvaluationContext(
    userId,
    seanceId,
    unit,
    variations,
    ...
    const historicalBeforeDate = prEvaluationOptions?.historicalBeforeDate;
    const prReferenceDate = prEvaluationOptions?.referenceDate != null
        ? new Date(prEvaluationOptions.referenceDate)
        : new Date();
    ...
    const [bwContext, rawHistoricalSets] = await Promise.all([
        loadPrBodyweightEnrichmentContext(userId, variations),
        fetchPersonalRecordCandidateSets(
            userId,
            seanceId,
            unit,
            variations,
    ...
    const historicalSets = rawHistoricalSets.map((set, index) => {
...
function evaluatePersonalRecordWithPreloadedContext(
    context,
    {
        value,
        weightLoad,
    ...
    try {
        ...
        const setsAtSameValue = filterSetsAtSameValue(
            historicalSameUnit,
            currentValue,
        );
        ...
        if (isAth) {
        ...
        } else if (!hasHistoryAtLoad && !hasHistoryAtValue) {
            status = "NB";
            statusReason = 'noHistoricalAtValueOrLoad';
        } else {
            ...
            if (beatsValue || beatsLoad) {
                status = "PR";
                if (beatsValue && beatsLoad) {
                    statusReason = 'higherValueAndLoad';
                } else if (beatsValue) {
                    statusReason = 'higherValueAtSameLoad';
                } else {
                    statusReason = 'higherLoadAtSameValue';
                }
            } else if (tiesValue && tiesLoad) {
                status = "SB";
                statusReason = 'sameValueAtSameLoad';
            } else if (!tiesValue && hasHistoryAtLoad) {
                statusReason = 'belowHistoricalValueAtSameLoad';
            } else if (!tiesLoad && hasHistoryAtValue) {
        ...
        if (
            ...
            referenceBestSet = getReferenceBestSetAtSameReps(
                historicalSameUnit,
                currentValue,
            );
        }
        ...
        const {
            ...
            kgDelta,
            lbsDelta,
        } = resolvePrDetailDeltas({
            status,
            statusReason,
            unit,
            currentValue,
            currentEffectiveLoad,
            currentEffectiveLoadLbs,
            referenceBestSet,
            referenceEffectiveLoad,
            referenceEffectiveLoadLbs,
        });
...
async function evaluatePersonalRecordsBatch({
    userId,
    seanceId,
    variations,
    prEvaluationOptions = undefined,
    ...
    for (const setInput of sets) {
        const key = getPrContextKey(setInput);
        if (!groups.has(key)) {
            groups.set(key, []);
        }
        groups.get(key).push(setInput);
    }

    const results = [];
    for (const groupSets of groups.values()) {
        const sample = groupSets[0];
        const context = await loadPrEvaluationContext(
            userId,
            seanceId,
            sample.unit,
            variations,
            sample.isUnilateral,
            sample.unilateralSide,
            prEvaluationOptions,
        );

        for (const setInput of groupSets) {
            const numericValue = Number(setInput.value);
            if (setInput.unit === 'repetitions' && Number.isFinite(numericValue) && numericValue === 0) {
                results.push({
                    setId: setInput.setId,
                    isPersonalRecord: null,
                    prDetail: null,
                });
                continue;
            }
            if (setInput.unit === 'cardio' && setInput.value === 0) {
                results.push({
                    setId: setInput.setId,
                    isPersonalRecord: null,
                    prDetail: null,
            ...
            const evaluation = evaluatePersonalRecordWithPreloadedContext(context, {
                value: setInput.value,
                weightLoad: setInput.weightLoad,
                elastic: setInput.elastic,
                effectiveWeightLoadOverride: setInput.effectiveWeightLoadOverride,
                cardio: setInput.cardio,
                sessionSets,
                excludeSetId: setInput.setId,
            });
            results.push({
                setId: setInput.setId,
                ...evaluation,
...
async function evaluatePersonalRecordWithContext(
    ...
    if (unit === 'repetitions' && Number.isFinite(numericValue) && numericValue === 0) {
        logPrEval('result', { status: null, reason: 'zeroRepetitionsNoBadge' });
        return { isPersonalRecord: null, prDetail: null };
    }

    if (unit === 'cardio' && value === 0) {
        return { isPersonalRecord: null, prDetail: null };
    }
    ...
    logPrEval('request', {
        ...
        value,
        weightLoad,
        effectiveLoad: effectiveWeightLoadOverride != null && Number.isFinite(Number(effectiveWeightLoadOverride))
            ? round2(Number(effectiveWeightLoadOverride))
            : round2(getEffectiveLoad({ weightLoad, elastic })),
        isUnilateral,
        unilateralSide,
        excludeSetId: excludeSetId != null ? String(excludeSetId) : null,
        sessionPeerCount: Array.isArray(sessionSets) ? sessionSets.length : 0,
        historicalBeforeDate: prEvaluationOptions?.historicalBeforeDate != null
            ? String(prEvaluationOptions.historicalBeforeDate)
            : null,
        prReferenceDate: prEvaluationOptions?.referenceDate != null
            ? new Date(prEvaluationOptions.referenceDate).toISOString()
    ...
    if (!context.isCardio) {
        logPrEval('historyLoaded', {
            historicalCount: context.historicalSets.length,
            sessionPeerCount: Array.isArray(sessionSets) ? sessionSets.length : 0,
            historicalSample: context.historicalSets
    ...
    const result = evaluatePersonalRecordWithPreloadedContext(context, {
        value,
        weightLoad,
        elastic,
        effectiveWeightLoadOverride,
    ...
    logPrEval('result', {
        status: result.isPersonalRecord,
        statusReason: result.prDetail?.statusReason ?? null,
    });
...
/**
 * Deltas prDetail affichables côté UI, y compris les cas non-PR.
 * Bi-axial : valeur à charge L et charge à valeur V.
 */
function resolvePrDetailDeltas({
    status = null,
    statusReason = null,
    unit = 'repetitions',
    currentValue = null,
    currentEffectiveLoad = null,
    currentEffectiveLoadLbs = null,
    referenceBestSet = null,
    referenceEffectiveLoad = null,
    referenceEffectiveLoadLbs = null,
}) {
    let repsDelta = null;
    ...
    if (!referenceBestSet) {
        return { repsDelta, secondsDelta, kgDelta, lbsDelta };
    }

    const setZeroValueDelta = () => {
        if (unit === 'seconds') {
            secondsDelta = 0;
        } else {
            repsDelta = 0;
    ...
    const setValueDeltaFromReference = () => {
        const referenceValue = Number(referenceBestSet.value);
        if (!Number.isFinite(referenceValue) || !Number.isFinite(currentValue)) return;
    ...
    const setLoadDeltaFromReference = () => {
        if (referenceEffectiveLoad == null || !Number.isFinite(currentEffectiveLoad)) return;
        kgDelta = round2(currentEffectiveLoad - referenceEffectiveLoad);
        if (referenceEffectiveLoadLbs != null && Number.isFinite(currentEffectiveLoadLbs)) {
            lbsDelta = round2(currentEffectiveLoadLbs - referenceEffectiveLoadLbs);
        } else if (kgDelta != null) {
            lbsDelta = round2(kgDelta * KG_TO_LB);
        }
    };

    if (statusReason === 'higherValueAtSameLoad') {
        setValueDeltaFromReference();
    } else if (statusReason === 'higherLoadAtSameValue') {
        setLoadDeltaFromReference();
    } else if (statusReason === 'higherValueAndLoad') {
        setValueDeltaFromReference();
        setLoadDeltaFromReference();
    } else if (statusReason === 'sameValueAtSameLoad') {
        setZeroValueDelta();
        kgDelta = 0;
        lbsDelta = 0;
    } else if (statusReason === 'belowHistoricalValueAtSameLoad') {
        setValueDeltaFromReference();
        kgDelta = 0;
        lbsDelta = 0;
    } else if (statusReason === 'belowHistoricalLoadAtSameValue') {
        setZeroValueDelta();
        setLoadDeltaFromReference();
    }

    if (!status && (statusReason == null || statusReason === 'noHistoricalAtValueOrLoad')) {
        return { repsDelta, secondsDelta, kgDelta, lbsDelta };
    }
...
module.exports = {
    ...
    resolveSharedFamilyPrFetchForDetailedPrs,

...
const { compareAndAssignPR, getEffectiveLoad, getEffectiveLoadPreferringPersisted, resolvePrComparisonOneRmKg, LOAD_EPSILON } = require('../utils/set');
const {
...
/**
 * Déplie une variation combo (equivalentTo) pour unifier le graphe
 * ex. set "Dips anneaux" vs set "Dips + Anneaux".
 */
function resolveChartSourceVariationIds(sourceVariationIds, variationById) {
    ...
    const doc = variationById?.get?.(String(ids[0])) ?? variationById?.[String(ids[0])];
    const equivalentTo = Array.isArray(doc?.equivalentTo)
        ? doc.equivalentTo.map((id) => String(id)).filter(Boolean)
        : [];
    if (equivalentTo.length >= 2) return equivalentTo;
    return ids;
}
...
async function resolveFigureRecommendationAllowlist({
    ...
    const orderedVariationIds = [...uniqueFamilyRowIds];
    ...
    if (!capped) {
        ...
        const graphOnly = edgeVariationIds
            .map(String)
            .filter((id) => !familyIdSet.has(id));
        const graphSlots = Math.max(0, safeMax - uniqueFamilyRowIds.length);
        targetVariationIds = [
            ...uniqueFamilyRowIds,
    ...
    for (const id of targetVariationIds) {
        if (!familyIdSet.has(String(id))) edgeIdSet.add(String(id));
    }
...
async function collectTargetSignaturesForProgressionFamily({
    ...
    const variationById = await buildVariationByIdMapFromSetsAsync(familySets, extraIds);
    ...
    try {
        ...
        if (isAth) {
            ...
            const maxLoadAtValue = setsSameValue.reduce(
                (max, s) => Math.max(max, getEffectiveLoadPreferringPersisted(s)),
                0,
            );
            if (currentEffectiveLoad > maxLoadAtValue + LOAD_EPSILON) {
                status = "PR";
        ...
        const referenceBestSet = getReferenceBestSetAtSameReps(historicalSameUnit, value);

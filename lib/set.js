...
const { filterOnDatesAgg } = require('../controllers/utils/aggregation');
require('dotenv').config();
const { compareAndAssignPR } = require('../controllers/utils/set');
const { invalidateCacheStartingWith, getOrSetCache } = require('../controllers/utils/cache');
...
async function getSets(userId, seanceId, exercice, categories, unit, value, weightLoad, elastic) {
    try {
        const cacheKey = `sets_${userId || ''}_${seanceId || ''}_${exercice || ''}_${categories || ''}_${unit || ''}_${value || ''}_${weightLoad || ''}_${elastic || ''}`;
...
async function getTopExercices(userId, by, asc) {
    try {
        return await getOrSetCache(`topExercices:${userId}:${by}:${asc}`, async () => {
            let groupBy = "$seance";
            let sort = -1
            ...
            const agg = [
                { $match: { user: mongoose.Types.ObjectId(userId) } }, ...filterOnDatesAgg(userId),
 ...
 */
async function getTopFormat(userId) {
    try {
        const cacheKey = `topFormat_${userId}`;
        return await getOrSetCache(cacheKey, async () => {
            const agg = [
                // Step 1: Match documents by user ID
                { $match: { user: mongoose.Types.ObjectId(userId) } },

                // Step 2: Apply date filtering stages returned by filterOnDatesAgg
                ...filterOnDatesAgg(userId),
                ...
                // Step 3: Group sets by exercise, categories, seance, and unit to create seance-specific formats
                {
                    $group: {
                        ...
                        sets: { $push: "$value" } // Push set values to create format
                    }
                },
                ...
                // Step 4: Format the sets array to show occurrences of each format
                {
                    $group: {
                        ...
                        occurrence: { $sum: 1 } // Count the occurrences of each format
                    }
                },
                ...
                // Step 8: Limit to top 5 results
                { $limit: 5 }
            ]
            const topFormats = await Set.aggregate(agg).exec();
            return topFormats;
        });
    } catch (err) {
...
async function getPRs(userId, exercice, categories) {
    try {
        const cacheKey = `prs_${userId}_${exercice || ''}_${categories ? JSON.stringify(categories) : ''}`;
        return await getOrSetCache(cacheKey, async () => {
...
async function createSet(setData) {
    try {
        ...
        await invalidateCacheStartingWith(`sets_${setData.user || ''}`);
        await invalidateCacheStartingWith(`topExercices:${setData.user || ''}`);
        await invalidateCacheStartingWith(`topFormat_${setData.user || ''}`);
        await invalidateCacheStartingWith(`prs_${setData.user || ''}`);
...
module.exports = { getSets, getTopExercices, createSet, getPRs, getTopFormat };

...
async function deleteSets(seanceId) {
    try {
        const existingSet = await Set.findOne({ seance: seanceId });
        const userId = existingSet?.user?.toString();

        await Set.deleteMany({ seance: seanceId });

        if (userId) {
            await invalidateSetCaches(userId);
            await invalidateSeanceCaches(userId, seanceId);
        }
    } catch (err) {

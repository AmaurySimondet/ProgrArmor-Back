...
async function getStats(userId, includeActive = true) {
    try {
        ...
        const shifts = await Shift.find(query);
        const stats = calculateShiftStats(shifts.filter(s => s.type !== 'off'), now);
        const offDays = getOffDaysFromShifts(shifts);
        ...
        const {
            ...
            expectedRemoteSeconds,
            expectedOfficeShifts,
            expectedOfficeSeconds,
            expectedTotalWorkDays,
            expectedSoFarNetSeconds,
            expectedSoFarRemoteShifts,
            expectedSoFarOfficeShifts,
            expectedSoFarTotalWorkDays,
            expectedSoFarRemoteSeconds,
            expectedSoFarOfficeSeconds
        } = computeYearExpectations({
            startOfYear,
            startOfNextYear,
            weekSchedule,
...
/**
 * Extract a set of day keys (ISO at midnight) that are marked as "off"
 * @param {Array} shifts - Array of shifts
 * @returns {Set<string>} - Set of ISO date strings representing off days
 */
function getOffDaysFromShifts(shifts) {
    const offDays = new Set();

    for (const shift of shifts) {
        if (shift.type !== 'off' || !shift.startedAt) continue;
        const start = new Date(shift.startedAt);
        const end = new Date(shift.endedAt);
        for (let date = start; date <= end; date.setDate(date.getDate() + 1)) {
            offDays.add(date.toISOString());
        }
    }
...
/**
 * Helper to format a duration in seconds into h/m/s + formatted string
...
/**
 * Helper to compute a percentage with 2 decimals
...
/**
 * Compute yearly expectations (full year + "so far") given a schedule and explicit off days
 * @param {Object} params
 * @param {Date} params.startOfYear
 * @param {Date} params.startOfNextYear
 * @param {Object} params.weekSchedule
 * @param {number} params.netWorkTimeMinimumSeconds
 * @param {number} params.breakDurationMinimumSeconds
 * @param {Date} params.now
 * @param {Set<string>} params.offDays
 * @returns {Object}
 */
function computeYearExpectations({
    startOfYear,
    startOfNextYear,
    weekSchedule,
    netWorkTimeMinimumSeconds,
...
async function getSchedule(userId) {
    try {
        ...
        // Adjust expectations for explicit "off" shifts within the week (past or future)
        const offDaysCurrentWeek = getOffDaysFromShifts(currentWeekShifts);
        for (const dayKey of offDaysCurrentWeek) {
            const offDate = new Date(dayKey);
            if (offDate < currentWeek.start || offDate > currentWeek.end) continue;

            const dayName = getDayName(offDate);
            const scheduleType = weekSchedule[dayName];
